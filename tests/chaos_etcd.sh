#!/bin/bash
# etcd 混沌测试 — 断连/重启/数据丢失恢复
# 用法: ./tests/chaos_etcd.sh  (需集群运行中)
set -euo pipefail
...
echo ""
echo "--- 0. 基线 (etcd 在线) ---"
check "etcd health"     'curl -sf --max-time 3 http://127.0.0.1:2379/health' '"health":"true"'
check "GenKey 正常"     'curl -sf --max-time 5 http://127.0.0.1:27008/Interface/gentoken -d "{\"option\":\"GenKey\"}"' '"code":0'
...
sleep 3
check "etcd 不可达"    'curl -sf --max-time 2 http://127.0.0.1:2379/health || echo "down"' 'down'
check "GenKey 仍可路由(缓存)" 'curl -sf --max-time 5 http://127.0.0.1:27008/Interface/gentoken -d "{\"option\":\"GenKey\"}" | python3 -c "import sys,json;print(json.load(sys.stdin).get(\"code\",1))"' '0'

echo "  启动 etcd..."
docker start thunder-deploy-etcd-1 >/dev/null 2>&1
sleep 5
check "etcd 恢复"      'curl -sf --max-time 3 http://127.0.0.1:2379/health' '"health":"true"'
sleep 5
check "registry 恢复"  'curl -s --max-time 3 http://127.0.0.1:2379/v3/kv/range -d "{\"key\":\"L3RodW5kZXIvcmVnaXN0cnkv\",\"range_end\":\"L3RodW5kZXIvcmVnaXN0cnkw\"}" | python3 -c "import sys,json;print(json.load(sys.stdin).get(\"count\",0))"' '3'
...
sleep 5
check "etcd 启动(空)"  'curl -sf --max-time 3 http://127.0.0.1:2379/health' '"health":"true"'
sleep 30
check "节点重新注册"   'curl -s --max-time 3 http://127.0.0.1:2379/v3/kv/range -d "{\"key\":\"L3RodW5kZXIvcmVnaXN0cnkv\",\"range_end\":\"L3RodW5kZXIvcmVnaXN0cnkw\"}" | python3 -c "import sys,json;print(json.load(sys.stdin).get(\"count\",0))"' '3'
    for i in $(seq 1 10); do c=$(curl -s --max-time 3 http://127.0.0.1:2379/v3/kv/range -d "{\"key\":\"L3RodW5kZXIvcmVnaXN0cnkv\",\"range_end\":\"L3RodW5kZXIvcmVnaXN0cnkw\"}" | python3 -c "import sys,json;print(json.load(sys.stdin).get(\"count\",0))" 2>/dev/null); [ "$c" = "3" ] && break; sleep 3; done
check "GenKey 恢复"    'curl -sf --max-time 5 http://127.0.0.1:27008/Interface/gentoken -d "{\"option\":\"GenKey\"}"' '"code":0'
