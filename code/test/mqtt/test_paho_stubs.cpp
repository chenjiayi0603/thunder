// Comprehensive stubs for paho-mqtt-c OS-level symbols needed by MQTT codec tests.
// Only symbols from paho utility files (Heap, Log, StackTrace, etc.) are resolved
// by compiling those .c files directly into the test object library.
#include <cstdio>
#include <cstdlib>
...
// === WebSocket (used by MQTTPacket.c) ===
...
// === Protocol (used by MQTTPersistence.c) ===
...
// === Socket / Proxy (used by MQTTPersistence.c protocol path) ===
...
// === Global state (used by MQTTPersistence.c) ===
int bstate = 0;
void* state = 0;
int (*clientSocketCompare)(int, int) = 0;

// === MQTTTime (stub header has inline, but real header found first) ===
