...
net::E_CMD_STATUS StepLocateData::Emit(int iErrno, const std::string& strErrMsg, const std::string& strErrClientShow)
{
    ...
    LOG4_TRACE("%s", oRequest.ToString().c_str());
    uint32 uiFactor = 0;
    ...
    */
    ...
    {
        Response(robot::ERR_LACK_CLUSTER_INFO, "tb_name or redis_key must be set!", "tb_name or redis_key must be set!");
    ...
    LOG4_TRACE("body: %s",oMsgBody.body().c_str());
    if (!GetLabor()->SendToNext(REDISAGENT_NODE, oMsgHead, oMsgBody))
    {
        LOG4_ERROR("send to dataproxy error!");
        Response(robot::ERR_SERVERINFO, "send to dataproxy error!", "send to dataproxy error!");
        return(net::STATUS_CMD_FAULT);
    }
...
{
    LOG4_TRACE("StepTestGet Timeout");
    HttpMsg oHttpMsg;
    oHttpMsg.set_type(HTTP_RESPONSE);
    oHttpMsg.set_status_code(200);
    oHttpMsg.set_http_major(m_oInHttpMsg.http_major());
...
bool StepLocateData::Response(int iErrno, const std::string& strErrMsg, const std::string& strErrClientShow)
{
    ...
    oHttpMsg.set_http_major(m_oInHttpMsg.http_major());
    oHttpMsg.set_http_minor(m_oInHttpMsg.http_minor());
    oJsonObj.Add("code", iErrno);
    oJsonObj.Add("msg", strErrClientShow.c_str());
    oHttpMsg.set_body(oJsonObj.ToFormattedString());
    return GetLabor()->SendTo(m_stInMsgShell, oHttpMsg);
}
