...
#define PG_TB_TEST "tb_test"
    void InsertPostgres(const net::tagMsgShell& stMsgShell,
                        const HttpMsg&          oInHttpMsg,
                        const std::string&      sValue,
                        const std::string&      nodeType);
    void SetGetPostgres(const net::tagMsgShell& stMsgShell,
                        const HttpMsg&          oInHttpMsg,
                        const std::string&      sValue,
                        const std::string&      nodeType);
    void GetPostgres(const net::tagMsgShell& stMsgShell,
                     const HttpMsg&          oInHttpMsg,
                     const std::string&      sValue,
                     const std::string&      nodeType);
    void SetPostgres(const net::tagMsgShell& stMsgShell,
                     const HttpMsg&          oInHttpMsg,
                     const std::string&      sValue,
                     const std::string&      nodeType);
    void AddUpPostgres(const net::tagMsgShell& stMsgShell,
                       const HttpMsg&          oInHttpMsg,
                       uint32                  id,
...
#define TEST_SSDB_KEY "1:2:testkey"
    // redis basic
    void SetValueFromRedis(const net::tagMsgShell& stMsgShell,
                           const HttpMsg&          oInHttpMsg,
                           const std::string&      sValue,
                           const std::string&      nodeType = PROXY);
    void OnlySetValueFromRedis(const net::tagMsgShell& stMsgShell,
                               const HttpMsg&          oInHttpMsg,
                               const std::string&      sValue,
                               const std::string&      nodeType = PROXY);
    void OnlyGetValueFromRedis(const net::tagMsgShell& stMsgShell,
                               const HttpMsg&          oInHttpMsg,
                               const std::string&      sValue,
                               const std::string&      nodeType = PROXY);

    // redisearch http://redisearch.io/Commands/
    void RedisearchAdd(const net::tagMsgShell& stMsgShell,
                       const HttpMsg&          oInHttpMsg,
    ...
    void
    RedisGEORADIUSBYMEMBER(const net::tagMsgShell& stMsgShell, const HttpMsg& oInHttpMsg, const std::string& sValue);

    // redis bitmap
#define SETBIT_KEY "4:4:SETBIT"
    void RedisbitmapSETBIT(const net::tagMsgShell& stMsgShell,
                           const HttpMsg&          oInHttpMsg,
                           const std::string&      sValue,
                           const std::string&      sKey  = SETBIT_KEY,
                           const std::string&      sNode = PROXY);
    void RedisbitmapGETBIT(const net::tagMsgShell& stMsgShell,
                           const HttpMsg&          oInHttpMsg,
                           const std::string&      sValue,
                           const std::string&      sKey  = SETBIT_KEY,
                           const std::string&      sNode = PROXY);
    void RedisbitmapBITPOS(const net::tagMsgShell& stMsgShell,
                           const HttpMsg&          oInHttpMsg,
                           const std::string&      sValue,
                           const std::string&      sKey  = SETBIT_KEY,
                           const std::string&      sNode = PROXY);
    void RedisbitmapGET(const net::tagMsgShell& stMsgShell,
                        const HttpMsg&          oInHttpMsg,
                        const std::string&      sValue,
                        const std::string&      sKey  = SETBIT_KEY,
                        const std::string&      sNode = PROXY);
    void RedisbitmapGET_GET(const net::tagMsgShell& stMsgShell,
                            const HttpMsg&          oInHttpMsg,
                            const std::string&      sValue,
                            const std::string&      sKey1 = SETBIT_KEY,
                            const std::string&      sKey2 = SETBIT_KEY,
                            const std::string&      sNode = PROXY);
...
#define MSG_KEY "1:11:MSG"
    void SsdbMsgHset(const net::tagMsgShell& stMsgShell,
                     const HttpMsg&          oInHttpMsg,
                     const std::string&      sValue,
                     const std::string&      sKey  = MSG_KEY,
                     const std::string&      sNode = PROXY);
    void SsdbMsgHsetall(const net::tagMsgShell& stMsgShell,
                        const HttpMsg&          oInHttpMsg,
                        const std::string&      sKey  = MSG_KEY,
                        const std::string&      sNode = PROXY);
    void SsdbMsgHscan(const net::tagMsgShell& stMsgShell,
                      const HttpMsg&          oInHttpMsg,
    ...
    bool TestHttpRequestStateFuncDataProxy(const net::tagMsgShell& stMsgShell,
                                           const HttpMsg&          oInHttpMsg,
