...
extern "C" {
    ...
    net::Cmd* create()
    {
        net::Cmd* pCmd = new net::CmdDbOper();
        return(pCmd);
    }
#ifdef __cplusplus
}
...
bool CmdDbOper::Init()
{
    ...
    DbAgentSession* pDbAgentSession = GetDbAgentSession(this->GetLabor());
    if (!pDbAgentSession)
    {
        LOG4_ERROR("GetDbAgentSession error!");
        return false;
    }
    pDbAgentSession->pCmdDbOper = this;
    return(true);
}
...
bool CmdDbOper::AnyMessage(const net::tagMsgShell& stMsgShell, const MsgHead& oInMsgHead, const MsgBody& oInMsgBody)
{
    ...
    if (bConnected)
    {
        ...
        if (0 == m_uiSync)
        {
			...
			{
				iResult = AsyncQuery(stMsgShell,oInMsgHead,oMemOperate, pSlaveDbi);
				if (0 == iResult)
				{//异步返回结果
					return(true);
				}
				...
				{
					iResult = AsyncQuery(stMsgShell,oInMsgHead,oMemOperate, pMasterDbi);
					if (0 == iResult)
					{//异步返回结果
						return(true);
					}
					...
					{
						Response(stMsgShell,oInMsgHead,pMasterDbi->GetErrno(), pMasterDbi->GetError());
						return(false);
					}
				}
			}
			else
			{
				if (NULL == pMasterDbi)
				{//异步返回结果
					Response(stMsgShell,oInMsgHead,pSlaveDbi->GetErrno(), pSlaveDbi->GetError());
					return(false);
				}
				iResult = AsyncQuery(stMsgShell,oInMsgHead,oMemOperate, pMasterDbi);
				if (0 == iResult)
...
bool CmdDbOper::GetDbConnection(const net::tagMsgShell& stMsgShell, const MsgHead& oInMsgHead,const DataMem::MemOperate& oQuery,
                util::CMysqlDbi** ppMasterDbi, util::CMysqlDbi** ppSlaveDbi)
{
    LOG4_TRACE("%s()", __FUNCTION__);
    char szFactor[32] = {0};
    int32 iDataType = 0;
    ...
    else
    {
        ...
        else
        {
            ...
            if (nArraySize > 0)
            {//集群
                if (!FetchOrEstablishConnectionForCluster(strInstance,oGroupHostConf,oInstanceConf, ppMasterDbi))
                {
                    Response(stMsgShell,oInMsgHead,ERR_NEW, "FetchOrEstablishConnectionForCluster failed!");
                    return(false);
                }
...
bool CmdDbOper::FetchOrEstablishConnectionForCluster(const std::string &strInstance,util::CJsonObject& oHostListConf,
                const util::CJsonObject& oInstanceConf, util::CMysqlDbi** ppMasterDbi)
{
    ...
    *ppMasterDbi = NULL;
            ...
            if (dbi_iter == m_mapDbiPool.end())
            {
                ...
                pConnection->pDbi = pDbi;
                {
                    ...
                    strncpy(stDbConfDetail.m_stDbConnInfo.m_szDbHost,
                                    strHost.c_str(),sizeof(stDbConfDetail.m_stDbConnInfo.m_szDbHost));
    ...
    if (*ppMasterDbi)
    {
        LOG4CPLUS_TRACE_FMT(GetLogger(), "%s() use mysql(%s,%u) connection",__FUNCTION__,
    ...
    {
        LOG4CPLUS_ERROR_FMT(GetLogger(), "%s() failed to get mysql connection",__FUNCTION__);
        return(false);
    }
}

bool CmdDbOper::FetchOrEstablishConnection(const net::tagMsgShell& stMsgShell, const MsgHead& oInMsgHead,
...
int CmdDbOper::ConnectDb(const util::CJsonObject& oInstanceConf, util::CMysqlDbi* pDbi,const std::string& strDbIdentify)
{
...
int CmdDbOper::ConnectDb(const util::tagDbConfDetail &stDbConfDetail, util::CMysqlDbi* pDbi)
{
    LOG4_DEBUG("%s()", __FUNCTION__);
    int iResult = 0;
    LOG4_DEBUG("InitDbConn(%s, %s, %s, %s, %u, %s)", stDbConfDetail.m_stDbConnInfo.m_szDbHost,
...
int CmdDbOper::SyncQuery(const net::tagMsgShell& stMsgShell,const MsgHead& oInMsgHead,const DataMem::MemOperate& oQuery, util::CMysqlDbi* pDbi)
{
    LOG4_TRACE("%s()", __FUNCTION__);
    if (NULL == pDbi)
    {
...
int CmdDbOper::AsyncQuery(const net::tagMsgShell& stMsgShell,const MsgHead& oInMsgHead,const DataMem::MemOperate& oQuery, util::CMysqlDbi* pDbi)
{
    LOG4_TRACE("%s()", __FUNCTION__);
    if (NULL == pDbi)
    {
    ...
    struct QueryMysqlParam:public net::StepStateParam//mysql访问参数
	{
    	QueryMysqlParam(CmdDbOper* pCmd,const DataMem::MemOperate& oReq,const std::string &sql,
    			const net::tagMsgShell &shell,const MsgHead &oHead,
				uint32 uiSectionFrom,uint32 uiSectionTo,uint32 uiHash,uint32 uiDivisor):
    		pCmdDbOper(pCmd),oQuery(oReq),strSql(sql),stMsgShell(shell),oInMsgHead(oHead),
    ...
    if (DataMem::MemOperate::DbOperate::SELECT == oQuery.db_operate().query_type())
    {
		...
		{
			STAGE_TEST_PARAM_LOG(QueryMysqlParam,state,"mysqlSelectCallback");
    ...
    else
    {
		...
		{
			STAGE_TEST_PARAM_LOG(QueryMysqlParam,state,"mysqlExecCallback");
			net::MysqlStep* pMysqlState = (net::MysqlStep*)state;
			pStageParam->pCmdDbOper->AsyncQueryCallback(pStageParam->oQuery,
					pMysqlState->m_pMysqlResSet,pMysqlState->m_pMysqlResSet->GetErrno(),pStageParam->strSql,
					pStageParam->stMsgShell,pStageParam->oInMsgHead,
...
int CmdDbOper::AsyncQueryCallback(const DataMem::MemOperate& oQuery, util::MysqlResSet* pMysqlResSet,int iResult,
...
//					}
	...
	else
	{
		...
		if (DataMem::MemOperate::DbOperate::SELECT != oQuery.db_operate().query_type()
						&& ((pMysqlResSet->GetErrno() >= 2001 && pMysqlResSet->GetErrno() <= 2018)
										|| (pMysqlResSet->GetErrno() >= 2038 && pMysqlResSet->GetErrno() <= 2046)))
		{   // 由于连接方面原因数据写失败，将失败数据节点返回给数据代理，等服务从故障中恢复后再由数据代理自动重试
...
void CmdDbOper::Response(const net::tagMsgShell &stMsgShell,const MsgHead &oInMsgHead,int iErrno, const std::string& strErrMsg)
{
...
bool CmdDbOper::Response(const net::tagMsgShell &stMsgShell,const MsgHead &oInMsgHead,const DataMem::MemRsp& oRsp)
{
...
std::string CmdDbOper::GetFullTableName(const std::string& strTableName, uint64 uiFactor)
{
...
bool CmdDbOper::CreateSql(const DataMem::MemOperate& oQuery, util::CMysqlDbi* pDbi, std::string& strSql)
{
    LOG4_TRACE("%s()",__FUNCTION__);
...
bool CmdDbOper::CreateInsert(const DataMem::MemOperate& oQuery, util::CMysqlDbi* pDbi, std::string& strSql)
{
    LOG4_TRACE("%s()",__FUNCTION__);
...
bool CmdDbOper::CreateUpdate(const DataMem::MemOperate& oQuery, util::CMysqlDbi* pDbi, std::string& strSql)
{
    LOG4_TRACE("%s()",__FUNCTION__);
...
bool CmdDbOper::CreateCondition(const DataMem::MemOperate::DbOperate::Condition& oCondition, util::CMysqlDbi* pDbi, std::string& strCondition)
{
    LOG4_TRACE("%s()",__FUNCTION__);
...
bool CmdDbOper::CreateConditionGroup(const DataMem::MemOperate& oQuery, util::CMysqlDbi* pDbi, std::string& strCondition)
{
    LOG4_TRACE("%s()",__FUNCTION__);
