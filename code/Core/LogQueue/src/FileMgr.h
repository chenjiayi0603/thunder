...
//日志记录列表
typedef std::vector<logqueue::log> LogMsgVec;
enum elog_cmd
{
	elog_cmd_trace = 1,
	elog_cmd_device = 2,
...
vecComsumeTables消费文件格式
{
	...
	"head":{"index":{"_index":"db_start_trace","_type":"tb_start_trace"}}
}
 * */
//本地文件管理类
class FileMgr
...
public:
    ...
    ~FileMgr();
    /*配置*/
    void Init(){SetCurrentTime();}
    uint32 SetCurrentTime()
    {
    	m_uiCurrentTime = ::time(NULL);
    	char sDate[32];
		util::GetDateStr(sDate,sizeof(sDate),m_uiCurrentTime);
		m_strDate = sDate;
    	return m_uiCurrentTime;
    }
    std::map<std::string,util::CJsonObject> m_oTableHead;//日志头部
    std::map<std::string,int> m_oTableCmd;//日志指令
    /*日志接口*/
    bool OpenLog();//打开日志文件
    bool AddLog(const logqueue::log &logMsg);//追加日志记录到日志队列追加列表
...
private:
    //自定义二进制头存储结构
    bool Write2CustomHeadLogFile(const logqueue::log& message);

    //类ES型存储结构消费文件
    bool Write2ESConsumeFile(const logqueue::log& message);
    bool Write2ESConsumeFileWithLog(const std::string& logBody,const logqueue::log& message);

    //ssdb存储ES型消费文件
    bool Write2SSDBConsumeFile(const logqueue::log& message);
    bool Write2SSDBConsumeFileWithLog(const std::string& logBody,const logqueue::log& message);
