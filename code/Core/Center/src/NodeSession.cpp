...
bool NodeSession::Init(std::string& err, bool boReload)
{
    ...
    { // center
        ...
        for (int i = 0; i < s; ++i)
        {
            ...
            LOG4_DEBUG("load center_inner_host(%s),center_inner_port(%d),server_identify(%s)",
                       center_inner_host.c_str(),
                       center_inner_port,
                       centerServer.server_identify.c_str());
    ...
    { // 中心服务器本身配置
        ...
        m_CenterActive.status     = eMasterStatus; // eOfflineStatus;
        if (!LoadNodeRoute())
        {
    ...
    if (!m_objRoute.IsEmpty())
    {
        if (m_vecWhiteNode.size() == 0)
        {
            ...
            if (m_objRoute.Get("ipwhite", objIpwhite))
            {
                ...
                for (int i = 0; i < s; ++i)
                {
                    snprintf(whiteNode.inner_ip,
                             sizeof(whiteNode.inner_ip),
                             RemoveFlagString(objIpwhite[i].ToString()).c_str());
        ...
        if (m_vecNodeTypes.size() == 0)
        {
            ...
            if (m_objRoute.Get("auto", objAutoNodetype)) //"auto" ,优先自动路由
            {
                ...
                for (auto node : vecNode)
                {
                    ...
                    for (auto neededServer : nodeType.neededServers)
                        LOG4_INFO("neededServer:%s", neededServer.c_str());
...
int NodeSession::RegNode(const net::tagMsgShell& stMsgShell,
                         ...
                         const NodeStatusInfo&   nodeinfo)
{
    ...
    {
        LOG4_DEBUG("Master(%s,%d) RegNodeRoute(%s)",
                   m_CenterActive.inner_ip,
                   m_CenterActive.inner_port,
    ...
    else // 更新上报状态
    {
        if (!pNodeInfo->update(nodeinfo))
        {
            LOG4_WARN("pNodeInfo(%s,%d,%s,%d) update failed",
                      ...
                      pNodeInfo->nodeId,
                      pNodeInfo->nodeInnerIp.c_str(),
                      pNodeInfo->nodeInnerPort);
...
int NodeSession::RegNodeRoute(const net::tagMsgShell& stMsgShell,
                              const MsgHead&          oInMsgHead,
                              const MsgBody&          oInMsgBody,
                              const NodeStatusInfo&   nodeinfo)
{
    ...
    if (pNodeInfo) // 注册过的只需要更新节点信息
    {
        ...
        LOG4_DEBUG("Master(%s,%d) getNodeKey(%s)",
                   m_CenterActive.inner_ip,
                   m_CenterActive.inner_port,
    ...
    else // 没有注册过的需要检查,然后分配节点并注册
    {
        ...
        LOG4_DEBUG("Master(%s,%d) getNodeKey(%s),new nodeId(%d)",
                   m_CenterActive.inner_ip,
                   m_CenterActive.inner_port,
                   nodeinfo.getNodeKey().c_str(),
                   tmpRegNodeStatus.nodeId);
...
bool NodeSession::ResToReg(const net::tagMsgShell& stMsgShell,
                           const MsgHead&          oInMsgHead,
                           int                     iRet,
                           const NodeStatusInfo&   regNodeStatus)
{
...
int NodeSession::SendRegToOthers(const NodeStatusInfo& regNodeStatus)
{
    ...
    {
        LOG4_TRACE("regNodeStatus(%s,%s) is suspend",
                   regNodeStatus.getNodeKey().c_str(),
                   regNodeStatus.nodeType.c_str());
        return (ERR_OK);
    }
    const std::string& strRegNodeType = regNodeStatus.nodeType;
    // 发送格式
    // {\"node_arry_reg\":[{\\"node_type\\":\"ACCESS\",\\"node_ip\\":\"192.168.18.22\",\\"node_port\\":40111,\\"worker_num\\":2}]}
    ...
    for (NodesStatusMapCIT it_iter = m_mapNodesStatus.begin(); it_iter != m_mapNodesStatus.end();
         ++it_iter) // 已注册服务器
    {
        ...
        if (pNodeType) // 已注册的节点需要的节点类型
        {
            if (pNodeType->neededServers.size() > 0)
            {
                ...
                for (NodeType::ServersListCIT it = vecServersType.begin(); it != vecServersType.end(); ++it)
                {
                    ...
                    {                          // 通知已注册的服务
                        net::SendToCallback(this,
                                            net::CMD_REQ_NODE_REG_NOTICE,
                                            toNoticeMsgBody,
                                            defaultCallback,
                                            info.getNodeKey());
                    }
            ...
            {
                LOG4_DEBUG("registed node type(%s) don't need other node routes", info.nodeType.c_str());
            }
        }
        else
        {
            LOG4_WARN("node type(%s) don't have such server,please check table tb_nodetype", info.nodeType.c_str());
        }
    }
    return (ERR_OK);
}

int NodeSession::SendCenterToReg(const net::tagMsgShell& stMsgShell,
                                 const MsgHead&          oInMsgHead,
                                 const MsgBody&          oInMsgBody,
                                 const NodeStatusInfo&   regNodeStatus)
{
    ...
    if (pNodeType)
    {
        ...
        if (!net::SendToCallback(this,
                                 net::CMD_REQ_NODE_REG_NOTICE,
                                 objRegNodes.ToString(),
                                 defaultCallback,
                                 stMsgShell))
        {
...
bool NodeSession::GetNeededNodesStatus(const std::vector<std::string>& neededServers, util::CJsonObject& jObj)
{
    ...
    for (const auto& it_iter : m_mapNodesStatus) // 已注册服务器
    {
        ...
        {
            LOG4_TRACE("need nodetype[%s] checking online nodetype[%s] suspend(%d)",
                       it.c_str(),
                       pInfo->nodeType.c_str(),
                       pInfo->suspend);
            ...
            {
                LOG4_TRACE("need nodetype[%s] skip online nodetype[%s] suspend(%d)",
                           it.c_str(),
                           pInfo->nodeType.c_str(),
                           pInfo->suspend);
            }
        ...
        for (const auto& it_iter : m_mapNodesStatus) // 已注册的服务器
        {
            ...
            if (pNodeType)
            {
                for (const auto& it : pNodeType->neededServers) // 该类服务器需要的服务器类型
                {
                    ...
                    {
                        LOG4_DEBUG("%s() nodeInfo(%s)!oExitNode[%s]",
                                   __FUNCTION__,
                                   nodeInfo.getNodeKey().c_str(),
                                   oExitNode.ToString().c_str());
                        if (!net::SendToCallback(this,
                                                 net::CMD_REQ_NODE_REG_NOTICE,
                                                 strDisConnectBody,
                                                 defaultCallback,
            ...
            {
                LOG4_WARN("%s() nodeInfo(%s) don't have server config,please check config file",
                          __FUNCTION__,
...
int NodeSession::SendOfflineToGate(const NodeStatusInfo& offlineNodeInfo)
{
    ...
    // 遍历管理器内存的node列表,如果断开连接的服务是它们需要的服务,则通知它们注销该断开连接的服务
    {
        ...
        // 在线节点管理器（key为节点类型：IP：端口，value为节点信息）
        for (NodeSession::NodesStatusMapCIT it_iter = m_mapNodesStatus.begin(); it_iter != m_mapNodesStatus.end();
             ++it_iter) // 已注册的服务器
        {
            const NodeStatusInfo& nodeInfo = it_iter->second;
            ...
            if (pNodeType)
            {
                ...
                for (std::vector<std::string>::const_iterator it = neededServers.begin(); it != neededServers.end();
                     ++it) // 该类服务器需要的服务器类型
                {
            ...
            {
                LOG4_WARN(
                    "SendOfflineToGate!notify node type(%s) don't have server config,please check table tb_nodetype",
                    nodeInfo.nodeType.c_str());
            }
        }
    }
...
NodeSession* GetNodeSession(bool boReload)
{
    ...
    LOG4CPLUS_INFO_FMT(g_pLabor->GetLogger(),
                       "new NodeSession(1,%d) NodeSession timeout:%d",
                       nodeSessionTimeOut,
                       nodeSessionTimeOut);
    std::string err;
    if (!pSess->Init(err))
