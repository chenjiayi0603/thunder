...
StepRestartWorkers::StepRestartWorkers(const net::tagMsgShell &stMsgShell,const MsgHead& oInMsgHead,
...
net::E_CMD_STATUS StepRestartWorkers::Emit(int iErrno,
                const std::string &strErrMsg, const std::string &strErrShow)
{
    MsgHead oOutMsgHead;
    MsgBody oOutMsgBody;
    oOutMsgHead.set_cmd(net::CMD_REQ_NODE_RESTART_WORKERS);
...
net::E_CMD_STATUS StepRestartWorkers::Callback(const net::tagMsgShell &stMsgShell,
...
net::E_CMD_STATUS StepRestartWorkers::Timeout()
{
    ++m_iTimeOut;
    if(m_iTimeOut < 5)
    {
...
net::E_CMD_STATUS StepRestartWorkers::Response(int iErrno)
{
    /*
                服务器节点上线响应
