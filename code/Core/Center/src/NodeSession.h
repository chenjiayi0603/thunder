...
public:
    NodeSession(uint32 ulSessionId, ev_tstamp dSessionTimeout, const std::string& strSessionName = "net::NodeSession")
        : Timer(ulSessionId, dSessionTimeout, strSessionName), boInit(false), m_nCheckActiveCounter(0),
          m_nNodeTimeBeat(0), m_uiNodeId(0), m_uiCurrentTime(0), m_uiInitSessionTime(0), m_centerInnerPort(0),
          m_centerProcessNum(0)
    {
        SetCurrentTime();
...
public:
    ...
    bool LoadNodeRoute();
    // 注册节点
    int RegNode(const net::tagMsgShell& stMsgShell,
    ...
    // 注册成功会发送以下消息：	//(1)发送返回注册响应,会分配节点id	//(2)发送注册服务器的配置信息(注册成功后才调用)
    // //(3)发送其他服务器给注册者、发送注册者给其它服务
    // 注册返回信息：（1）注册响应,会分配节点id（2）发送注册服务器的配置信息(注册成功后才调用)（3）给注册者发其他服务器通知、注册者的给其它服务发通知
    // //返回注册响应,会分配节点id
    int RegNodeRoute(const net::tagMsgShell& stMsgShell,
                     const MsgHead&          oInMsgHead,
                     const MsgBody&          oInMsgBody,
                     const NodeStatusInfo&   nodeinfo);
    // 检查节点类型
    ...
    // 发送返回注册响应,会分配节点id
    bool ResToReg(const net::tagMsgShell& stMsgShell,
                  const MsgHead&          oInMsgHead,
                  int                     iRet,
    ...
    // 发送中心服务器给注册者
    int SendCenterToReg(const net::tagMsgShell& stMsgShell,
                        const MsgHead&          oInMsgHead,
                        const MsgBody&          oInMsgBody,
                        const NodeStatusInfo&   regNodeStatus);
    // 发送连接断开通知到其它服务
    ...
    bool IsMaster() const
    {
        return (eMasterStatus == m_CenterActive.status);
    }
    bool IsSlave() const
    {
        return (eSlaveStatus == m_CenterActive.status);
    }
    // 获取自身的标识符ip:port
...
private:
    ...
    /*路由配置，如：
     * "center_inner_host":"192.168.18.68",  "center_inner_port":15000,  "center_node_type":"CENTER",
