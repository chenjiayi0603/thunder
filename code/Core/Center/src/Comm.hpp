...
typedef struct NodeStatusInfo
{
    ...
    bool pareJsonData(util::CJsonObject& jObj)
    {
        ...
        if (jObj.Get("node_type", nodeType) && jObj.Get("node_ip", nodeInnerIp) &&
            jObj.Get("node_port", nodeInnerPort) && jObj.Get("worker_num", workerNum) && jObj.Get("node_id", nodeId) &&
            jObj.Get("node", nodeobj) && jObj.Get("worker", workerobj))
        {
            // 数据作检查
            if (nodeType == "" || nodeInnerPort == 0 || workerNum == 0 || nodeInnerIp == "")
            {
                return false;
            }
...
typedef struct NodeLogInfo
{
    ...
    {
        LOG4_DEBUG(
            "nodeId(%d),nodeType(%s),nodeInnerPort(%d),nodeInnerIp(%s),nodeAccessPort(%d),nodeAccessIp(%s),"
            ...
            activeTime, // 活跃时间
            worker
                .c_str(), // 负载描述,如：[{"load":8,"connect":8,"recv_num":1,"recv_byte":14,"send_num":1,"send_byte":14,"client":5}
            // 统计数据
            addUpLoad,       // 服务器负载累计
            counterLoad,     // 服务器负载计数
...
typedef struct NodesStaisticsInfo
{
    ...
    NodesStaisticsInfo& operator+=(const NodesStaisticsInfo& info)
    {
        if ((nodeType == info.nodeType) && (nodeInnerIp == info.nodeInnerIp) && (nodeInnerPort == info.nodeInnerPort))
        {
            recvNum += info.recvNum;
            recvByte += info.recvByte;
            sendNum += info.sendNum;
