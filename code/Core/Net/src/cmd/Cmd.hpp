...
protected:
     ...
     * @return Server自定义配置
     */
    const util::CJsonObject& GetCustomConf() const;
    ...
    bool SendBack(const net::tagMsgShell& oInMsgShell,const std::string &strBody,int iCode=200,bool gzip = false);//http1.1
    bool SendBack(const net::tagMsgShell& oInMsgShell,const HttpMsg& oInHttpMsg,const std::string &strBody,int iCode=200);//http
	bool SendBack(const net::tagMsgShell& oInMsgShell,const MsgHead &oInMsgHead,const std::string &strBody);//pb
    //接入服务器使用的对外接口
	...
	bool SendToModCallBack(Session* pSession,uint32 uiCmd,const std::string &strBody,
					...
					const std::string &nodeType);
	//远程回调
	bool SendToProxyCallBack(net::Step* pUpperStep,const DataMem::MemOperate* pMemOper,StorageCallbackStep callback,
			const std::string &nodeType="PROXY",uint32 uiCmd = net::CMD_REQ_STORATE);
	bool SendToProxyModCallBack(net::Step* pUpperStep,const DataMem::MemOperate* pMemOper,StorageCallbackStep callback,int uiModFactor,
				const std::string &nodeType="PROXY",uint32 uiCmd = net::CMD_REQ_STORATE);
	bool SendToCallBack(net::Step* pUpperStep,uint32 uiCmd,const std::string &strBody,StandardCallbackStep callback,
					const std::string &nodeType);
	...
	bool CoroutineResumeWithTimes(uint32 nMaxTimes=0){return GetLabor()->CoroutineResumeWithTimes(nMaxTimes);}
