...
protected:
    ...
    bool SendToProxyCallBack(const DataMem::MemOperate* pMemOper,
                    StorageCallbackSession callback,bool boPermanentSession,
                const std::string &nodeType="PROXY",uint32 uiCmd = net::CMD_REQ_STORATE);
    bool SendToProxyModCallBack(const DataMem::MemOperate* pMemOper,StorageCallbackSession callback,bool boPermanentSession,int uiModFactor,
                const std::string &nodeType="PROXY",uint32 uiCmd = net::CMD_REQ_STORATE);
    bool SendToCallBack(uint32 uiCmd,const std::string &strBody,
                    StandardCallbackSession callback,bool boPermanentSession,
                    const std::string &nodeType);
    bool SendToModCallBack(uint32 uiCmd,const std::string &strBody,
