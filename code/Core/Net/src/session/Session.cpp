...
const util::CJsonObject& Session::GetCustomConf() const
{
...
bool Session::SendToProxyCallBack(const DataMem::MemOperate* pMemOper,
        StorageCallbackSession callback,bool boPermanentSession,
        const std::string &nodeType,uint32 uiCmd)
{
    return GetLabor()->SendToProxyCallBack(this,pMemOper,callback,boPermanentSession,nodeType,uiCmd,-1);
}

bool Session::SendToProxyModCallBack(const DataMem::MemOperate* pMemOper,StorageCallbackSession callback,bool boPermanentSession,int uiModFactor,
                const std::string &nodeType,uint32 uiCmd)
{
    return GetLabor()->SendToProxyCallBack(this,pMemOper,callback,boPermanentSession,nodeType,uiCmd,uiModFactor);
}

bool Session::SendToCallBack(uint32 uiCmd,const std::string &strBody,
        StandardCallbackSession callback,bool boPermanentSession,
        const std::string &nodeType)
{
    return GetLabor()->SendToCallBack(this,uiCmd,strBody,callback,boPermanentSession,nodeType,-1);
}

bool Session::SendToModCallBack(uint32 uiCmd,const std::string &strBody,
                StandardCallbackSession callback,bool boPermanentSession,int uiModFactor,
                const std::string &nodeType)
{
    return GetLabor()->SendToCallBack(this,uiCmd,strBody,callback,boPermanentSession,nodeType,uiModFactor);
}
