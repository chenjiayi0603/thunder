...
pstep->AddStateFunc(net::StateSendToIdentify);//stage 0
pstep->AddStateFunc(net::StateSendToIdentifyCallback);//stage 1
pstep->SetData(new net::SendToIdentifyParam(sNodeIdentify,strConfig,net::CMD_REQ_SERVER_CONFIG));
...
bool StepState::Launch(Labor* pLabor,StepState *step,uint32 uiTimeOutMax,uint8 uiToRetry,double dTimeout)
{
	if (step == NULL)
	{
...
void StepState::AsyncSend(const net::tagMsgShell& stMsgShell,const std::string& strBody,int iCmd,StateFunc callback)
{
	AddStateFunc(StateSendToMsgShell);//stage 0
	AddStateFunc(callback);//stage 1
	SetData(new net::SendToMsgShellParam(stMsgShell,strBody,iCmd));
...
void StepState::AsyncSend(const std::string &strToIdentify,const std::string& strBody,int iCmd,StateFunc callback)
{
	...
	AddStateFunc(callback);//stage 1
	SetData(new net::SendToIdentifyParam(strToIdentify,strBody,iCmd));
	m_strStepDesc = std::string("StepState:") + strToIdentify + " " + __FUNCTION__;
}
void StepState::AsyncSend(const net::tagMsgShell& stMsgShell,const MsgHead& oMsgHead, const MsgBody& oMsgBody,StateFunc callback)
{
	AddStateFunc(StateSendPbToMsgShell);//stage 0
	AddStateFunc(callback);//stage 1
	SetData(new net::SendPbToMsgShellParam(stMsgShell,oMsgHead,oMsgBody));
	m_strStepDesc = std::string("StepState:") + __FUNCTION__;
}
void StepState::AsyncSend(const std::string &strToIdentify,const MsgHead& oMsgHead, const MsgBody& oMsgBody,StateFunc callback)
{
	...
	AddStateFunc(callback);//stage 1
	SetData(new net::SendPbToIdentifyParam(strToIdentify,oMsgHead,oMsgBody));
	m_strStepDesc = std::string("StepState:") + strToIdentify + " " + __FUNCTION__;
}

bool StepState::Register(Labor* pLabor,StepState *step,uint32 uiTimeOutMax,uint8 uiToRetry,double dTimeout)
{
	if (step == NULL)
	{
		LOG4CPLUS_ERROR_FMT(pLabor->GetLogger(),"%s() null step",__FUNCTION__);
