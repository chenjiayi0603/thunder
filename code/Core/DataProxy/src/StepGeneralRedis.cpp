...
E_CMD_STATUS StepGeneralRedis::Emit(int iErrno, const std::string& strErrMsg, const std::string& strErrShow)
{
    ...
    if (pRedisCmd && pRedisCmd->GetCmd().size() == 0)
    {
        ...
        ++m_iEmitNum;
        UnsetRegistered();
        if (RegisterCallback(m_strNode, this))
        {
...
E_CMD_STATUS StepGeneralRedis::Callback(const redisAsyncContext *c, int status, redisReply* pReply)
{
    LOG4_TRACE("%s()", __FUNCTION__);
    char szErrMsg[256] = {0};
    if (REDIS_OK != status)
