...
bool SessionSyncDbData::SyncData(const DataMem::MemOperate& oMemOperate)
{
    ...
    if (NULL == m_pCodec)
    {
        m_pCodec = new ProtoCodec(util::CODEC_PROTOBUF);
    ...
    snprintf(szWriteFileName, sizeof(szWriteFileName), "%u.%s.%s.dat", GetWorkerIndex(),
                            ...
                            util::time_t2TimeStr(GetNowTime(), "YYYYMMDDHHMI").c_str());
...
SessionSyncDbData* GetSessionSyncDbData(Labor* pNodeLabor,
		const std::string& strTableName, const std::string& strDataPath, ev_tstamp dSessionTimeout)
{
	SessionSyncDbData* pSessionSyncDbData = (SessionSyncDbData*)pNodeLabor->GetSession(strTableName, "net::SessionSyncDbData");
