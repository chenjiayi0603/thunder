...
class SessionSyncDbData: public oss::Session
{
public:
...
private:
    ...
    loss::CBuffer* m_pBuffR;
    loss::CBuffer* m_pBuffWaitR;
    loss::CBuffer* m_pBuffW;
    oss::ProtoCodec* m_pCodec;
    int m_iSendingSize;
    int m_iReadingFd;
    int m_iWritingFd;
...
SessionSyncDbData* GetSessionSyncDbData(OssLabor* pNodeLabor,
		const std::string& strTableName, const std::string& strDataPath, ev_tstamp dSessionTimeout=10.0);
