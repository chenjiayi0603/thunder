...
SessionRedisNode::SessionRedisNode(const std::string& strSessionId,
                int iHashAlgorithm, int iVirtualNodeNum,const std::string &strGroupUse,ev_tstamp dSessionTimeout)
    : oss::Session(strSessionId, dSessionTimeout, "oss::SessionRedisNode"),
...
E_CMD_STATUS SessionRedisNode::Timeout()
{
    for(auto node:m_mapRedisNode)
    {
        char sKey[64] ={0};
        char sReadCmd[64] ={0};
        ...
        {//master
            oss::RedisOperator oRedisOperator(0, sKey,"",sReadCmd);
            ...
            Pretreat(pStepGeneralRedis);
            if (STATUS_CMD_RUNNING != pStepGeneralRedis->Emit(ERR_OK))
            {
...
//    uiKeyHash = *(uint32_t*)szDigest;
    ...
    LOG4_TRACE("%s() strNodeIdentify(%s) strMasterNodeIdentify(%s) strSlaveNodeIdentify(%s)",
            __FUNCTION__,strNodeIdentify.c_str(),strMasterNodeIdentify.c_str(),strSlaveNodeIdentify.c_str());
    return(true);
}
