...
StepOfflineNode::StepOfflineNode(const oss::tagMsgShell &stMsgShell,const MsgHead& oInMsgHead,
...
oss::E_CMD_STATUS StepOfflineNode::Emit(int iErrno,
                const std::string &strErrMsg, const std::string &strErrShow)
{
    MsgHead oOutMsgHead;
    MsgBody oOutMsgBody;
    oOutMsgHead.set_cmd(oss::CMD_REQ_NODE_STOP);
...
oss::E_CMD_STATUS StepOfflineNode::Callback(const oss::tagMsgShell &stMsgShell,
...
oss::E_CMD_STATUS StepOfflineNode::Timeout()
{
    ++m_iTimeOut;
    if(m_iTimeOut < 5)
    {
...
oss::E_CMD_STATUS StepOfflineNode::Response(int iErrno)
{
    /*
