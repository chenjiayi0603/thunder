...
class NodeSession: public oss::Timer
{
public:
    NodeSession(uint32 ulSessionId, ev_tstamp dSessionTimeout,
                    const std::string& strSessionName="oss::NodeSession"):
    ...
    struct LoadConfigSendToMysqlParam:public oss::StepStateParam//mysql访问参数
	{
    ...
    bool LoadConfigFiles(loss::T_vecResultSet &vecRes);
    //初始化
...
public:
     ...
     * */
    //注册节点
    int RegNode(const oss::tagMsgShell& stMsgShell, const MsgHead& oInMsgHead,
                        const MsgBody& oInMsgBody, const NodeStatusInfo& nodeinfo);
    //更新节点
    int UpdateNode(const oss::tagMsgShell& stMsgShell,
    ...
    //上报数据负载
    int WriteServerDataLoad(const oss::tagMsgShell& stMsgShell,
                    const MsgHead& oInMsgHead, const MsgBody& oInMsgBody);
public:
    //对外服务功能接口
    //获取最小负载节点
...
public:
    ...
    //检查配置内容
    int CheckServerConfigContent(const NodeConfigFile &nodeConfigFile,const loss::CJsonObject &checkConfigContent,
...
private:
    ...
    //加载节点类型
    bool LoadNodeTypes();
    bool LoadNodeTypes(loss::T_vecResultSet &vecRes);
    //加载服务器白名单
    bool LoadServerWhiteList();
    bool LoadServerWhiteList(loss::T_vecResultSet &vecRes);
    //检查中心活跃信息
    bool CheckCenterActive();
    bool CheckCenterActive(loss::T_vecResultSet &vecRes);
    ...
    //获取指定类型节点的在线数量
    oss::uint32 GetNodeCountByType(const std::string &nodeType);
    //添加最近负载
...
private:
    ...
    //返回注册响应,会分配节点id
    int RealRegNode(const oss::tagMsgShell& stMsgShell,
                    const MsgHead& oInMsgHead, const MsgBody& oInMsgBody,
					const NodeStatusInfo& nodeinfo);
    ...
    ///////注册返回信息函数///////
    //发送返回注册响应,会分配节点id
    bool ResponseNodeReg(const oss::tagMsgShell& stMsgShell,
                    const MsgHead& oInMsgHead, const MsgBody& oInMsgBody,
                    int iRet,const NodeStatusInfo &regNodeStatus);
    //发送注册服务器的配置信息(注册成功后才调用)
    bool SendServerConfig(const oss::tagMsgShell& stMsgShell,
    ...
    //发送其他服务器给注册者
    int SendOthersNoticeToRegNode(const oss::tagMsgShell& stMsgShell,const NodeStatusInfo &regNodeStatus);
    //发送注册者给其它服务
    int SendNodeRegNoticeToOthers(const NodeStatusInfo &regNodeStatus);
    //发送中心服务器给注册者
    int SendCenterNoticeToRegNode(const oss::tagMsgShell& stMsgShell,
                    const MsgHead& oInMsgHead, const MsgBody& oInMsgBody,const NodeStatusInfo &regNodeStatus);
    //发送连接断开通知到其它服务
    ...
    //获取中心服务器的节点状态(json格式)
    bool GetCenterNodesStatus(loss::CJsonObject &jObj);
private:
...
private:
     ...
     * 网络相关函数
     * */
    inline bool SendTo(const oss::tagMsgShell& stMsgShell, const MsgHead& oMsgHead, const MsgBody& oMsgBody)
    {
    ...
    * */
    ...
    //获取 mysql连接对象
    inline loss::CMysqlDbi* GetSyncMysqlDbi() const
    {
...
private:
    ...
    loss::tagDbConnInfo m_dbConnInfo;
    //节点分配器
    uint32 m_nodeId;
    //当前时间
     ...
     *数据库连接配置,如：
       ...
       "dbcharacterset":"utf8",
     * */
    loss::CMysqlDbi* m_pSyncMysqlDbi; //数据库连接(同步,目前只有运维操作使用)
    /*
    * 路由配置，如：
    ...
    * */
    ...
    oss::uint64 m_nodeLoadLogInsertLastTime;
    //节点负载统计最后写表tb_nodeload_statistics的时间
    //写表时间间隔m_nodeLoadStatisticsTimeInterval（300s）
    oss::uint64 m_nodeLoadStatisticsInsertLastTime;
    //节点负载最后检查时间
    ...
    oss::uint64 m_nodeLoadCheckLastTime;
    //节点状态最后检查时间
    ...
    oss::uint64 m_nodeStatusCheckLastTime;
    //节点数据负载最后检查时间
    ...
    oss::uint64 m_serverDataLoadCheckLastTime;
};

NodeSession* GetNodeSession(oss::OssLabor* pLabor,const std::string &configPath,bool boReload=false);
