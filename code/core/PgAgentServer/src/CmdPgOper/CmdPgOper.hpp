...
protected:
    ...
    int ConnectDb(const loss::CJsonObject& oInstanceConf, pqxx::connection* &pPgConn,const std::string& strDbIdentify);
    int ConnectDb(const loss::tagDbConfDetail &stDbConfDetail, pqxx::connection* &pPgConn);
    int SyncQuery(const oss::tagMsgShell& stMsgShell,const MsgHead& oInMsgHead,const DataMem::MemOperate& oQuery, pqxx::connection* pPgConn);
