...
extern "C" {
    ...
    oss::Cmd* create()
    {
        oss::Cmd* pCmd = new oss::CmdLocatePgData();
        return(pCmd);
    }
#ifdef __cplusplus
}
...
bool CmdLocatePgData::AnyMessage(const tagMsgShell& stMsgShell, const MsgHead& oInMsgHead, const MsgBody& oInMsgBody)
{
    ...
    else
    {
        ...
        else
        {
            ...
            loss::CJsonObject& dbInstanceConf = dbGroupConf[strInstance];
            std::string strUseGroup = dbInstanceConf("use_group");
			loss::CJsonObject& oGroupHostConf = dbInstanceConf[strUseGroup];
            ...
            oRspJson.Add("db_node", loss::CJsonObject("{}"));
...
std::string CmdLocatePgData::GetFullTableName(const std::string& strTableName, uint32 uiFactor)
{
...
void CmdLocatePgData::Response(const tagMsgShell& stMsgShell, const MsgHead& oInMsgHead,
