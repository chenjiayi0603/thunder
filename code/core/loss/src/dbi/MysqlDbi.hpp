...
class CMysqlDbi : public loss::CDbi
{
public:
    CMysqlDbi();
    CMysqlDbi(
    ...
    //初始化连接实例
    virtual int InitDbConn(const loss::tagDbConfDetail& stDbConfDetail);
...
public:
     ...
     * loss::CMysqlDbi::InitDbConn(const loss::tagDbConfDetail& stDbConfDetail)
     * 中的使用。
     */
...
private:
    ...
    loss::T_vecDict m_vecDict;	//结果集数据字典
    loss::T_mapRow m_mapRow;	//结果集行

    loss::tagDbConfDetail m_stDbConfDetail; // 数据库连接信息
 };
