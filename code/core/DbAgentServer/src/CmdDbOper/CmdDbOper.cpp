...
bool CmdDbOper::GetDbConnection(const oss::tagMsgShell& stMsgShell, const MsgHead& oInMsgHead,const DataMem::MemOperate& oQuery,
                loss::CMysqlDbi** ppMasterDbi, loss::CMysqlDbi** ppSlaveDbi)
{
    ...
    else
    {
        ...
        else
        {
            ...
            loss::CJsonObject& dbGroupConf = m_oDbConf["db_group"];
            ...
            loss::CJsonObject& oInstanceConf = dbGroupConf[strInstance];
            std::string strUseGroup = oInstanceConf("use_group");
            loss::CJsonObject& oGroupHostConf = oInstanceConf[strUseGroup];
...
bool CmdDbOper::FetchOrEstablishConnectionForCluster(const std::string &strInstance,loss::CJsonObject& oHostListConf,
...
bool CmdDbOper::FetchOrEstablishConnection(const oss::tagMsgShell& stMsgShell, const MsgHead& oInMsgHead,
...
int CmdDbOper::ConnectDb(const loss::CJsonObject& oInstanceConf, loss::CMysqlDbi* pDbi,const std::string& strDbIdentify)
{
...
int CmdDbOper::ConnectDb(const loss::tagDbConfDetail &stDbConfDetail, loss::CMysqlDbi* pDbi)
{
    LOG4_DEBUG("%s()", __FUNCTION__);
    int iResult = 0;
    LOG4_DEBUG("InitDbConn(%s, %s, %s, %s, %u, %s)", stDbConfDetail.m_stDbConnInfo.m_szDbHost,
...
int CmdDbOper::SyncQuery(const oss::tagMsgShell& stMsgShell,const MsgHead& oInMsgHead,const DataMem::MemOperate& oQuery, loss::CMysqlDbi* pDbi)
{
    LOG4_TRACE("%s()", __FUNCTION__);
    if (NULL == pDbi)
    {
...
int CmdDbOper::AsyncQuery(const oss::tagMsgShell& stMsgShell,const MsgHead& oInMsgHead,const DataMem::MemOperate& oQuery, loss::CMysqlDbi* pDbi)
{
    LOG4_TRACE("%s()", __FUNCTION__);
    if (NULL == pDbi)
    {
    ...
    struct QueryMysqlParam:public oss::StepStateParam//mysql访问参数
	{
    	QueryMysqlParam(CmdDbOper* pCmd,const DataMem::MemOperate& oReq,const std::string &sql,
    			const oss::tagMsgShell &shell,const MsgHead &oHead,
    ...
    if (DataMem::MemOperate::DbOperate::SELECT == oQuery.db_operate().query_type())
    {
    	auto mysqlSelectCallback = [](oss::StepState* state)
		{
    ...
    {
    	auto mysqlExecCallback = [](oss::StepState* state)
		{
...
int CmdDbOper::AsyncQueryCallback(const DataMem::MemOperate& oQuery, loss::MysqlResSet* pMysqlResSet,int iResult,
...
void CmdDbOper::Response(const oss::tagMsgShell &stMsgShell,const MsgHead &oInMsgHead,int iErrno, const std::string& strErrMsg)
{
...
bool CmdDbOper::Response(const oss::tagMsgShell &stMsgShell,const MsgHead &oInMsgHead,const DataMem::MemRsp& oRsp)
{
