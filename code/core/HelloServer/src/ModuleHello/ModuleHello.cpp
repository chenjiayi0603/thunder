...
oss::Cmd* create()
{
    oss::Cmd* pCmd = new starshiplib::ModuleHello();
    return(pCmd);
}
#ifdef __cplusplus
}
...
//        {
//            time = loss::TimeStr2time_t("2017-12-31 23:59:59");
//        }
                ...
                const HttpMsg& oInHttpMsg)
{
	...
	else if ("HttpsGet" == strOption)
	{
		...
		GetLabor()->HttpsGet(strVal,strResponse,"");//loss::CurlClient::eContentType_gzip
        ...
        const HttpMsg& oInHttpMsg,const std::string &sValue,const std::string &nodeType)
{
	...
	auto callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
	{
		...
		if (oRsp.err_no() == 0)
		{
			auto GetValueFromPostgres_callback = [] (const DataMem::MemRsp &oMemRsp,oss::Step* pStep)
			{
 ...
 * */
void ModuleHello::SetValueFromRedis(const oss::tagMsgShell& stMsgShell,
        const HttpMsg& oInHttpMsg,const std::string &sValue,const std::string &nodeType)
{
    struct DataStepCustom:public oss::DataStepParam
    {
	...
	auto callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
	{
		...
		if (oRsp.err_no() == 0)
		{
			auto GetValueFromRedis_callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
			{
			...
			oss::RedisOperator oRedisOperator(0, sRedisKey,"","GET");
	...
	char sRedisKey[64];
	snprintf(sRedisKey,sizeof(sRedisKey),"1:2:testkey");
	oss::RedisOperator oRedisOperator(0, sRedisKey,"SET");
...
Shortest transaction:           0.01
 * */
void ModuleHello::OnlySetValueFromRedis(const oss::tagMsgShell& stMsgShell,
        const HttpMsg& oInHttpMsg,const std::string &sValue,const std::string &nodeType)
{
	auto SetValueFromRedis_callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
	{
	...
	if (!SendToProxyCallBack(new oss::DataStep(stMsgShell,oInHttpMsg),oRedisOperator.MakeMemOperate(),
 ...
 * */
void ModuleHello::OnlyGetValueFromRedis(const oss::tagMsgShell& stMsgShell,
        const HttpMsg& oInHttpMsg,const std::string &sValue,const std::string &nodeType)
{
	auto SetValueFromRedis_callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
	{
 ...
 * */
void ModuleHello::RedisearchAdd(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg,const std::string &sDoc,const std::string &sValue)
{
	...
	FT.SEARCH IDX "你好"
	 * */
	auto RedisearchAdd_callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
	{
	...
	oss::RedisOperator oRedisOperator(0, "IDX",sRedisWriteCmd,"");
...
FT.SEARCH {index} {query} [NOCONTENT] [VERBATIM] [NOSTOPWORDS] [WITHSCORES] [WITHPAYLOADS] [WITHSORTKEYS]
  ...
  [LIMIT offset num]
 * */
void ModuleHello::RedisearchSearch(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg,const std::string &sValue)
{
	/*
	./src/redis-cli FT.ADD IDX docCn3 1.0 LANGUAGE chinese FIELDS txt "你好罗朋友"
	./src/redis-cli FT.SEARCH IDX "你好"
	 * */
	auto RedisearchSearch_callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
	{
	...
	oss::RedisOperator oRedisOperator(0, "IDX","",sRedisReadCmd);
	oRedisOperator.AddRedisField("",sValue);
	if (!SendToProxyCallBack(new oss::DataStep(stMsgShell,oInHttpMsg),oRedisOperator.MakeMemOperate(),
 ...
 * */
void ModuleHello::RedisGEOADD(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg,const std::string &sValue)
{
	auto RedisGEOADD_callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
	{
	...
	oss::RedisOperator oRedisOperator(0, "Guangdong-cities",sRedisCmd);
...
5) 1) "Qingyuan"
   2) "144.2208"
 * */
void ModuleHello::RedisGEORADIUSBYMEMBER(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg,const std::string &sValue)
{
	auto RedisGEORADIUSBYMEMBER_callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
	{
		LOG4CPLUS_TRACE_FMT(pStep->GetLogger(),"RedisearchAdd_callback %s",oRsp.err_msg().c_str());
		if (oRsp.err_no() == 0)
		{
...
void ModuleHello::RedisbitmapSETBIT(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg,const std::string &sValue,const std::string &sKey,const std::string &sNode)
{
	auto callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
	{
	...
	oss::RedisOperator oRedisOperator(0, sKey.size() > 0?sKey:SETBIT_KEY,"SETBIT");
	...
	SendToProxyCallBack(new oss::DataStep(stMsgShell,oInHttpMsg),oRedisOperator.MakeMemOperate(),callback,sNode);
}
//GETBIT 4:4:SETBIT 10001
void ModuleHello::RedisbitmapGETBIT(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg,const std::string &sValue,const std::string &sKey,const std::string &sNode)
{
    auto callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
    {
    ...
    oss::RedisOperator oRedisOperator(0, sKey.size() > 0?sKey:SETBIT_KEY,"GETBIT");
    oRedisOperator.AddRedisField("",sValue);//10001
    LOG4_DEBUG("%s() RedisbitmapGETBIT %s",__FUNCTION__,sValue.c_str());
    SendToProxyCallBack(new oss::DataStep(stMsgShell,oInHttpMsg),oRedisOperator.MakeMemOperate(),callback,sNode);
}
//BITPOS 4:4:SETBIT 1
void ModuleHello::RedisbitmapBITPOS(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg,const std::string &sValue,const std::string &sKey,const std::string &sNode)
{
    auto callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
    {
    ...
    oss::RedisOperator oRedisOperator(0, sKey.size() > 0?sKey:SETBIT_KEY,"BITPOS");
 ...
 * */
void ModuleHello::RedisbitmapGET(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg,const std::string &sValue,const std::string &sKey,const std::string &sNode)
{
    auto callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
    {
...
//                如果需要指定数量（或者指定分页，则只要使用usersData.size() 来控制）
    ...
    oss::RedisOperator oRedisOperator(0, sKey.size() > 0?sKey:SETBIT_KEY,"","GET");
    LOG4_DEBUG("%s() RedisbitmapGET",__FUNCTION__);
    SendToProxyCallBack(new oss::DataStep(stMsgShell,oInHttpMsg),oRedisOperator.MakeMemOperate(),callback,sNode);
}

void ModuleHello::RedisbitmapGET_GET(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg,
                const std::string &sValue,const std::string &sKey1,const std::string &sKey2,const std::string &sNode)
{
    ...
    auto callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
    {
        ...
        if (oRsp.err_no() == 0)
        {
            ...
            if (oRsp.record_data_size() > 0 && oRsp.record_data(0).field_info_size() > 0)
            {
                ...
                oss::RedisOperator oRedisOperator(0, ((DataStepCustom*) pDataStep->GetData())->sKey2,"","GET");
    ...
    oss::RedisOperator oRedisOperator(0, sKey1.size() > 0?sKey1:SETBIT_KEY,"","GET");
    LOG4_DEBUG("%s() RedisbitmapGET usersData1",__FUNCTION__);
    SendToProxyCallBack(new oss::DataStep(stMsgShell,oInHttpMsg,new DataStepCustom(sKey2.size() > 0?sKey2:SETBIT_KEY,sNode,GetLogger())),
...
void ModuleHello::SsdbMsgHset(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg,const std::string &sValue,const std::string &sKey,const std::string &sNode)
{
    auto callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
    {
    ...
    snprintf(strStorageKey,sizeof(strStorageKey),"%s?%u",sKey.size() > 0?sKey.c_str():MSG_KEY,loss::GetDateUint32(::time(NULL)));//1:11:MSG?20111101
    oss::RedisOperator oRedisOperator(0, strStorageKey,"HSET");
    oRedisOperator.AddRedisField("",loss::GetUniqueId(GetLabor()->GetNodeId(),GetLabor()->GetWorkerIndex()));
    oRedisOperator.AddRedisField("",sValue);// 1:11:MSG   {json}
    LOG4_DEBUG("%s() SsdbMsgHset %s,%s",__FUNCTION__,strStorageKey,sValue.c_str());
    SendToProxyCallBack(new oss::DataStep(stMsgShell,oInHttpMsg),oRedisOperator.MakeMemOperate(),callback,sNode);
}

void ModuleHello::SsdbMsgHsetall(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg,const std::string &sKey,const std::string &sNode)
{
    auto callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
    {
    ...
    char strStorageKey[64];
    snprintf(strStorageKey,sizeof(strStorageKey),"%s?%u",sKey.size() > 0?sKey.c_str():MSG_KEY,loss::GetDateUint32(::time(NULL)));//1:11:MSG?20111101
    oss::RedisOperator oRedisOperator(0, sKey.size() > 0?sKey:MSG_KEY,"HGETALL");
    LOG4_DEBUG("%s() SsdbMsgHsetall %s",__FUNCTION__,strStorageKey);
    SendToProxyCallBack(new oss::DataStep(stMsgShell,oInHttpMsg),oRedisOperator.MakeMemOperate(),callback,sNode);
}

void ModuleHello::SsdbMsgHscan(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg,const std::string &key_start,const std::string &sKey,const std::string &sNode)
{
    auto callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
    {
    ...
    char strStorageKey[64];
    snprintf(strStorageKey,sizeof(strStorageKey),"%s?%u",sKey.size() > 0?sKey.c_str():MSG_KEY,loss::GetDateUint32(::time(NULL)));//1:11:MSG?20111101
    oss::RedisOperator oRedisOperator(0, sKey.size() > 0?sKey:MSG_KEY,"hscan");
...
void ModuleHello::TestDBSELECT(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg)
{
	loss::CJsonObject oRequest;
	...
	auto TestDBSELECT_callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
	{
	...
	oss::DbOperator oDbOper(0, strTableName, DataMem::MemOperate::DbOperate::SELECT);
	...
	if (!SendToProxyCallBack(new oss::DataStep(stMsgShell,oInHttpMsg),oDbOper.MakeMemOperate(),
...
void ModuleHello::TestStepCoFuncDataProxy(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg,const std::string &str)
{
    struct StateParam:public oss::StepStateParam
    {
        StateParam(const std::string &s,const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg):
            str(s),shell(stMsgShell),msg(oInHttpMsg){}
        std::string str;
        oss::tagMsgShell shell;
        HttpMsg msg;
    };
    auto stateFunc0= [] (oss::StepCo* state)
    {
        {
            auto SetValueFromRedis_callback = [] (const DataMem::MemRsp &oRsp,oss::Step* pStep)
            {
    ...
    oss::StepCo* pstep = new oss::StepCo(stMsgShell,oInHttpMsg);
    pstep->AddCoroutinueFunc(stateFunc0);
    pstep->SetSuccFunc([] (oss::StepCo* state)
                    {
    ...
    oss::StepCo::Launch(GetLabor(),pstep);
}

void ModuleHello::Response(const oss::tagMsgShell& stMsgShell,
...
bool ModuleHello::TestHttpRequestState(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg)
{
     // HttpState
    return oss::StepState::Launch(GetLabor(),new StepHttpRequestState(stMsgShell,oInHttpMsg));
}

bool ModuleHello::TestHttpRequestStateFunc(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg)
{
	struct StateParam:public oss::StepStateParam
	{
	...
	auto stateFunc0= [] (oss::StepState* state)
	{
	...
	auto stateFunc1= [] (oss::StepState* state)
	{
	    LOG4CPLUS_DEBUG_FMT(state->GetLogger(), "%s GetLastState:%u GetCurrentState(%u) val(%u) m(%u)",
	                            __FUNCTION__,state->GetLastState(),state->GetCurrentState(),
	                        ((StateParam*)state->GetData())->Inc(),((StateParam*)state->GetData())->m.size());
	...
	auto stateFunc2= [] (oss::StepState* state)
	{
	    LOG4CPLUS_DEBUG_FMT(state->GetLogger(), "%s GetLastState:%u GetCurrentState(%u) val(%u) m(%u)",
	                            __FUNCTION__,state->GetLastState(),state->GetCurrentState(),
	                        ((StateParam*)state->GetData())->Inc(),((StateParam*)state->GetData())->m.size());
	...
	auto stateFunc3 = [] (oss::StepState* state)
	{
	    LOG4CPLUS_DEBUG_FMT(state->GetLogger(), "%s GetLastState:%u GetCurrentState(%u) val(%u) m(%u)",
	                            __FUNCTION__,state->GetLastState(),state->GetCurrentState(),
	                        ((StateParam*)state->GetData())->Inc(),((StateParam*)state->GetData())->m.size());
	...
	auto  stateFuncOnSucc = [] (oss::StepState* state)
	{
	    LOG4CPLUS_DEBUG_FMT(state->GetLogger(), "%s GetLastState:%u GetCurrentState(%u) val(%u) m(%u)",
	                            __FUNCTION__,state->GetLastState(),state->GetCurrentState(),
	                        ((StateParam*)state->GetData())->Inc(),((StateParam*)state->GetData())->m.size());
	...
	auto stateFuncOnFail = [] (oss::StepState* state)
	{
	    LOG4CPLUS_DEBUG_FMT(state->GetLogger(), "%s GetLastState:%u GetCurrentState(%u) val(%u) m(%u)",
	                        __FUNCTION__,state->GetLastState(),state->GetCurrentState(),
	                    ((StateParam*)state->GetData())->Inc(),((StateParam*)state->GetData())->m.size());
    ...
    return oss::StepState::Launch(GetLabor(),pstep);
}

bool ModuleHello::TestHttpRequestStateFuncDataProxy(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg,const std::string &str)
{
	struct StateParam:public oss::StepStateParam
	{
		StateParam(const std::string &s,const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg):
...
bool ModuleHello::TestJson2pb(const oss::tagMsgShell& stMsgShell,
                const HttpMsg& oInHttpMsg)
{
    /*
    ...
    */
    {//parse json to pb msg
        ...
        {//增加session
            loss::CJsonObject additionalJson;
