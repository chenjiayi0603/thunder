...
StepLocateData::StepLocateData(const oss::tagMsgShell& stInMsgShell, const HttpMsg& oInHttpMsg)
...
oss::E_CMD_STATUS StepLocateData::Emit(int iErrno, const std::string& strErrMsg, const std::string& strErrClientShow)
{
    MsgHead oMsgHead;
    MsgBody oMsgBody;
    loss::CJsonObject oRequest(m_oInHttpMsg.body());
    LOG4CPLUS_DEBUG_FMT(GetLogger(), "%s", oRequest.ToString().c_str());
    uint32 uiFactor = 0;
    ...
    */
    ...
    {
        uiFactor = loss::CalcKeyHash(oRequest("factor_string").c_str(), oRequest("factor_string").size());
    }
    if (oRequest.Get("tb_name", strTableName) && oRequest.Get("redis_key", strRedisKey))
    {
        oss::MemOperator oMemOper(uiFactor,
    ...
    {
        oss::DbOperator oDbOper(uiFactor, strTableName, DataMem::MemOperate::DbOperate::SELECT);
        oMsgBody.set_body(oDbOper.MakeMemOperate()->SerializeAsString());
    }
    else if (oRequest.Get("redis_key", strRedisKey))
    {
        oss::RedisOperator oRedisOper(uiFactor, strRedisKey, "get");
        oMsgBody.set_body(oRedisOper.MakeMemOperate()->SerializeAsString());
    }
    else
    {
        Response(oss::ERR_LACK_CLUSTER_INFO, "tb_name or redis_key must be set!", "tb_name or redis_key must be set!");
        return(oss::STATUS_CMD_FAULT);
    }
    oMsgHead.set_cmd(oss::CMD_REQ_LOCATE_STORAGE);
    oMsgHead.set_msgbody_len(oMsgBody.ByteSize());
    oMsgHead.set_seq(GetSequence());
    LOG4CPLUS_DEBUG_FMT(GetLogger(), "body: %s",oMsgBody.body().c_str());
    if (!SendToNext("PROXY", oMsgHead, oMsgBody))
    {
        LOG4CPLUS_ERROR(GetLogger(), "send to dataproxy error!");
...
oss::E_CMD_STATUS StepLocateData::Callback(
...
oss::E_CMD_STATUS StepLocateData::Timeout()
{
    LOG4CPLUS_DEBUG(GetLogger(), "StepTestGet Timeout");
    HttpMsg oHttpMsg;
    oHttpMsg.set_type(HTTP_RESPONSE);
    oHttpMsg.set_status_code(200);
    oHttpMsg.set_http_major(m_oInHttpMsg.http_major());
