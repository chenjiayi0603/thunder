...
class HelloSession: public oss::Session
{
public:
    HelloSession(double session_timeout = 100.0)
                    : oss::Session(HELLO_SESSIN_ID, session_timeout), boInit(false),
    ...
    const loss::CJsonObject & GetLocateDataRequest() {return m_objModuleLocateDataRequest;}
    //权限字段
...
private:
    ...
    loss::CJsonObject m_objModuleLocateDataRequest;
...
HelloSession* GetHelloSession(oss::OssLabor* pLabor,const std::string &configPath);
