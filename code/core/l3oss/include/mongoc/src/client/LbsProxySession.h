...
class LbsProxySession: public oss::Session
{
public:
    LbsProxySession(): oss::Session(LBS_PROXY_SESSIN_ID, LBS_PROXY_SESSION_TIME,
    ...
    int SearchDocs(const char*db, const char*tb,
                    ...
                    const loss::CJsonObject &fields, unsigned int skip = 0,
                    unsigned int limit = 0, unsigned int batch_size = 0);
    //更新一个文档
    //param 1:数据库名
    //param 2:表名(即集合collection)
    ...
    //param 4:返回结果数量
    //return 0则成功，其他则失败
    int Count(const char*db, const char*tb, const loss::CJsonObject &cond,
                    long long &count);
    //执行命令
    //param 1:数据库名
    //param 2:表名(即集合collection)
    ...
    //param 4：返回结果
    //return 0则成功，其他则失败
    int Command(const char*db, const char*tb, const loss::CJsonObject &command,
                    loss::CJsonObject &reply);
    /*逻辑接口*/
    //更新用户坐标
    ...
    //获取用户地理信息
    bool GetUserCoordinateInfo(unsigned int user_id,loss::CJsonObject &userObj);
    //获取群地理信息
    bool GetGroupCoordinateInfo(unsigned int group_id,loss::CJsonObject &groupObj);
    //删除用户坐标
    ...
    oss::int64 GetCurrentTime(){return m_currenttime;}
