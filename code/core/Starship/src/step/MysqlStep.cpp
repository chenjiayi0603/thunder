...
#include "labor/duty/OssWorker.hpp"
...
MysqlStep::MysqlStep(const loss::tagDbConnInfo &dbConnInfo)
{
...
MysqlStep::MysqlStep(const tagMsgShell& stReqMsgShell, const MsgHead& oReqMsgHead,const loss::tagDbConnInfo &dbConnInfo)
...
MysqlStep::MysqlStep(const tagMsgShell& stReqMsgShell, const HttpMsg& oInHttpMsg,const loss::tagDbConnInfo &dbConnInfo)
:StepState(stReqMsgShell,oInHttpMsg)
{
	Init(dbConnInfo.m_szDbHost,dbConnInfo.m_uiDbPort,dbConnInfo.m_szDbName,
				dbConnInfo.m_szDbUser,dbConnInfo.m_szDbPwd,dbConnInfo.m_szDbCharSet,dbConnInfo.uiTimeOut);
		...
		const std::string& user,const std::string& passwd,const std::string &dbcharacterset,uint32 uiTimeOut)
{
	...
	m_uiCmdType = 0;
	m_pMysqlResSet = new loss::MysqlResSet();
}

void MysqlStep::SetConf(const loss::tagDbConnInfo &dbConnInfo)
{
...
E_CMD_STATUS MysqlStep::Callback(loss::MysqlAsyncConn *c,loss::SqlTask *task,MYSQL_RES *pResultSet)
{
...
bool MysqlStep::Launch(OssLabor* pLabor,MysqlStep *pStep,uint32 uiTimeOutMax,uint8 uiToRetry,double dTimeout)
{
	if (pStep == NULL)
	{
...
int CustomMysqlHandler::on_execsql(loss::MysqlAsyncConn *c, loss::SqlTask *task) {
	++uiExecTaskCounter;
...
int CustomMysqlHandler::on_query(loss::MysqlAsyncConn *c, loss::SqlTask *task, MYSQL_RES *pResultSet) {
	++uiExecTaskCounter;
	LOG4_TRACE_S(((CustomMysqlHandler*) task->handler)->m_pNodeWorker,"%s:%d sql: %s done uiExecTaskCounter:%llu",
			__FUNCTION__,__LINE__, task->sql.c_str(),uiExecTaskCounter);
