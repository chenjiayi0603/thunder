...
public:
	...
	MysqlStep(const loss::tagDbConnInfo &dbConnInfo);
	MysqlStep(const tagMsgShell& stReqMsgShell, const MsgHead& oReqMsgHead,const loss::tagDbConnInfo &dbConnInfo);
	...
	MysqlStep(const tagMsgShell& stReqMsgShell, const HttpMsg& oInHttpMsg,const std::string& strHost, int iPort,const std::string& dbname,
			const std::string& user,const std::string& passwd,const std::string &dbcharacterset,uint32 uiTimeOut=3);
	MysqlStep(const tagMsgShell& stReqMsgShell, const HttpMsg& oInHttpMsg,const loss::tagDbConnInfo &dbConnInfo);
	void Init(const std::string& strHost, int iPort,const std::string& dbname,
			const std::string& user,const std::string& passwd,const std::string &dbcharacterset,uint32 uiTimeOut=3);

	virtual ~MysqlStep();
	void SetConf(const loss::tagDbConnInfo &dbConnInfo);
	void SetConf(const std::string& strHost, int iPort,const std::string& dbname,
			const std::string& user,const std::string& passwd,const std::string &dbcharacterset,uint32 uiTimeOut=3);
	static bool Launch(OssLabor* pLabor,MysqlStep *pStep,uint32 uiTimeOutMax=3,uint8 uiToRetry=1,double dTimeout=3.0);//开始步骤
     ...
     * @param pReply 执行结果集
     */
    virtual E_CMD_STATUS Callback(loss::MysqlAsyncConn *c,loss::SqlTask *task,MYSQL_RES *pResultSet);
    ...
    void SetTask(const std::string &strCmd,uint8 uiCmdType)
    {
    	...
    	if (loss::eSqlTaskOper_exec == uiCmdType)
    	{
    ...
    uint8 m_uiCmdType;
	//回调结果
	loss::MysqlResSet *m_pMysqlResSet;
};

class CustomMysqlHandler: public loss::MysqlHandler {
public:
	CustomMysqlHandler(MysqlStep* step,OssWorker* pNodeWorker):m_uiMysqlStepSeq(step->GetSequence()),m_pNodeWorker(pNodeWorker){}
	virtual ~CustomMysqlHandler(){}
	int on_execsql(loss::MysqlAsyncConn *c, loss::SqlTask *task);
	int on_query(loss::MysqlAsyncConn *c, loss::SqlTask *task, MYSQL_RES *pResultSet);
	uint32 m_uiMysqlStepSeq;
	OssWorker* m_pNodeWorker;
};
