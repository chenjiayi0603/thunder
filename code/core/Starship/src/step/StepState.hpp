...
public:
	...
	static bool Launch(OssLabor* pLabor,StepState *step,uint32 uiTimeOutMax=3,uint8 uiToRetry = 1,double dTimeout = 0.0);
	static bool Register(OssLabor* pLabor,StepState *step,uint32 uiTimeOutMax=3,uint8 uiToRetry = 1,double dTimeout = 0.0);
	StepState();
	...
	OssLabor* GetLabor(){return Step::GetLabor();}
	...
	void AsyncSend(const oss::tagMsgShell& stMsgShell,const std::string& strBody,int iCmd,StateFunc callback = StateSendToMsgShellCallback);//使用参数SendToMsgShellParam
	...
	void AsyncSend(const oss::tagMsgShell& stMsgShell,const MsgHead& oMsgHead, const MsgBody& oMsgBody,StateFunc callback = StateSendPbToMsgShellCallback);//使用参数SendPbToMsgShellParam
...
typedef oss::StepState::StateFunc StepStateFunc;
typedef oss::StepState::FinalFunc StepFinalFunc;
