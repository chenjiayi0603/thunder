...
StepNodeAccess::StepNodeAccess(const DataMem::MemOperate* pMemOper)
{
    Init();
    if (pMemOper)
    {
...
StepNodeAccess::StepNodeAccess(const std::string &strBody):m_strMsgSerial(strBody)
{
    Init();
}

StepNodeAccess::~StepNodeAccess()
{
}


void StepNodeAccess::Init()
{
    m_uiTimeOut = 0;
    m_uiTimeOutMax = 3;
    m_uiRetrySend = 1;
...
oss::E_CMD_STATUS StepNodeAccess::Emit(int iErrno , const std::string& strErrMsg , const std::string& strErrShow )
{
	if (m_uiCmd > 0 && m_strNodeType.size() > 0 && m_strMsgSerial.size() > 0)
	{
        ...
        LOG4CPLUS_TRACE_FMT(GetLogger(), "StepNodeAccess::Emit m_uiModFactor(%d)!Send cmd[%u] seq[%u] sending",
        ...
        if (!bRet)
        {
            LOG4CPLUS_ERROR_FMT(GetLogger(), "StepNodeAccess Send strNodeType(%s) failed.cmd[%u] seq[%u] send fail",
...
oss::E_CMD_STATUS StepNodeAccess::Timeout()
{
...
oss::E_CMD_STATUS StepNodeAccess::Callback(
        ...
        void* data)
{
    ...
    {
        if (oss::STATUS_CMD_RUNNING != m_pUpperStep->Emit())
        {
...
bool StepNodeAccess::DecodeMemRsp(DataMem::MemRsp &oRsp,const MsgBody& oInMsgBody)
{
	...
	if(0 != oRsp.err_no())
	{
		...
		{
			LOG4CPLUS_ERROR_FMT(GetLogger(), "StepNodeAccess::DecodeMemRsp error %d: %s!",
							oRsp.err_no(),oRsp.err_msg().c_str());
		}
		else
		{
			LOG4CPLUS_ERROR_FMT(GetLogger(), "StepNodeAccess::DecodeMemRsp error %d!",oRsp.err_no());
		}
		return false;
	}
	return true;
