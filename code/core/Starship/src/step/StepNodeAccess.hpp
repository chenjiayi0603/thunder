...
class StepNodeAccess: public oss::Step
{
public:
    StepNodeAccess(const DataMem::MemOperate* pMemOper);
    StepNodeAccess(const std::string &strBody);
    ...
    //Session对象异步访问存储回调
    void SetProxyCallBack(StorageCallbackSession callback,oss::Session* pSession,
    		bool boPermanentSession,const std::string &nodeType = "PROXY",uint32 uiCmd = oss::CMD_REQ_STORATE,int uiModFactor=-1)
    {
    ...
    //Step对象异步访问存储回调
    void SetProxyCallBack(StorageCallbackStep callback,oss::Step* pUpperStep,
    		const std::string &nodeType = "PROXY",uint32 uiCmd = oss::CMD_REQ_STORATE,int uiModFactor=-1)
	{
    ...
    //Session对象异步访问一般节点回调
    void SetCallBack(StandardCallbackSession callback,oss::Session* pSession,
    ...
    //Session对象异步访问一般节点回调
	void SetCallBack(StandardCallbackStep callback,oss::Step* pUpperStep,
...
private:
    ...
    oss::Session* m_pSession;
    oss::Step* m_pUpperStep;
...
//只是为了存储连接数据的步骤
class DataStep: public oss::HttpStep
{
public:
    DataStep(DataStepParam *data=NULL):m_data(data){}
	DataStep(const oss::tagMsgShell& stMsgShell,const HttpMsg& oInHttpMsg,DataStepParam *data=NULL)
        :stInMsgShell(stMsgShell),m_oInHttpMsg(oInHttpMsg),m_data(data){}
	DataStep(const oss::tagMsgShell& stMsgShell,const MsgHead& oInMsgHead,DataStepParam *data=NULL)
	:stInMsgShell(stMsgShell),m_oInMsgHead(oInMsgHead),m_data(data){}

    virtual ~DataStep(){if(m_data){delete m_data;m_data = NULL;}}
    virtual E_CMD_STATUS Emit(int iErrno = 0, const std::string& strErrMsg = "", const std::string& strErrShow = ""){return oss::STATUS_CMD_COMPLETED;}
    virtual E_CMD_STATUS Callback(const tagMsgShell& stMsgShell,const HttpMsg& oHttpMsg,void* data = NULL){return oss::STATUS_CMD_COMPLETED;}
	virtual E_CMD_STATUS Timeout(){return oss::STATUS_CMD_COMPLETED;}
