...
bool Cmd::SendBack(const oss::tagMsgShell& oInMsgShell,const std::string &strBody,int iCode,bool gzip)
{
	HttpMsg oHttpMsg;
...
bool Cmd::SendBack(const oss::tagMsgShell& oInMsgShell,const HttpMsg& oInHttpMsg,const std::string &strBody,int iCode)
{
	HttpMsg oHttpMsg;
	oHttpMsg.set_type(HTTP_RESPONSE);
	oHttpMsg.set_status_code(iCode);
...
bool Cmd::SendBack(const oss::tagMsgShell& oInMsgShell,const MsgHead &oInMsgHead,const std::string &strBody)
{
	MsgHead oOutMsgHead;
	MsgBody oOutMsgBody;
...
bool Cmd::SendToProxyCallBack(oss::Step* pUpperStep,const DataMem::MemOperate* pMemOper,StorageCallbackStep callback,
                const std::string &nodeType,uint32 uiCmd)
{
    return GetLabor()->SendToProxyCallBack(pUpperStep,pMemOper,callback,nodeType,uiCmd,-1);
}
bool Cmd::SendToProxyModCallBack(oss::Step* pUpperStep,const DataMem::MemOperate* pMemOper,StorageCallbackStep callback,int uiModFactor,
                const std::string &nodeType,uint32 uiCmd)
{
    return GetLabor()->SendToProxyCallBack(pUpperStep,pMemOper,callback,nodeType,uiCmd,uiModFactor);
}

bool Cmd::SendToCallBack(oss::Step* pUpperStep,uint32 uiCmd,const std::string &strBody,StandardCallbackStep callback,
                const std::string &nodeType)
{
    return GetLabor()->SendToCallBack(pUpperStep,uiCmd,strBody,callback,nodeType,-1);
}
bool Cmd::SendToModCallBack(oss::Step* pUpperStep,uint32 uiCmd,const std::string &strBody,StandardCallbackStep callback,int uiModFactor,
                const std::string &nodeType)
{
