...
{
    strLoadTime = loss::GetCurrentTime(20);
}
...
void OssWorker::RedisConnectCallback(const redisAsyncContext *c, int status)
{
    if (c->userData != NULL)
    {
        OssWorker* pWorker = (OssWorker*)c->userData;
...
void OssWorker::RedisClusterConnectCallback(const redisAsyncContext *c, int status)
{
    if (c->userData != NULL)
    {
        OssWorker* pWorker = (OssWorker*)c->userData;
        LOG4CPLUS_INFO_FMT(pWorker->GetLogger(),"%s",__FUNCTION__);
    }
}

void OssWorker::RedisClusterDisconnectCallback(const redisAsyncContext *c, int status)
{
    if (c->userData != NULL)
    {
        OssWorker* pWorker = (OssWorker*)c->userData;//目前当做集群是高可用的
        LOG4CPLUS_INFO_FMT(pWorker->GetLogger(),"%s",__FUNCTION__);
    }
}

void OssWorker::RedisClusterCmdCallback(redisClusterAsyncContext *acc, void *reply, void *privdata)
{
    if (privdata != NULL)
    {
        OssWorker* pWorker = (OssWorker*)privdata;
            ...
            while (conn_iter->second->pRecvBuff->ReadableBytes() >= gc_uiClientMsgHeadSize)
            {
                ...
                if (eConnectStatus_init == ptagConnectionAttr->ucConnectStatus)
                {
                    ...
                    if (CODEC_STATUS_ERR == eCodecStatus &&
                        (loss::CODEC_WEBSOCKET_EX_JS == ptagConnectionAttr->eCodecType
                        		|| loss::CODEC_WEBSOCKET_EX_PB == ptagConnectionAttr->eCodecType))
                    {
                        //切换为http协议
                        ...
                        std::map<loss::E_CODEC_TYPE, StarshipCodec*>::iterator codec_iter = m_mapCodec.find(ptagConnectionAttr->eCodecType);
                        if (codec_iter == m_mapCodec.end())
                        {
                    ...
                    if (CODEC_STATUS_ERR == eCodecStatus && loss::CODEC_HTTP == ptagConnectionAttr->eCodecType)
                    {
                ...
                if (CODEC_STATUS_OK == eCodecStatus)
                {
                    ...
                    if (oInMsgHead.cmd() > 0)
                    {
                        ...
                        else
                        {
                            ...
                            if (loss::CODEC_WEBSOCKET_EX_PB == ptagConnectionAttr->eCodecType ||
                                            loss::CODEC_WEBSOCKET_EX_JS == ptagConnectionAttr->eCodecType)
                            {
...
bool OssWorker::OnRedisConnect(const redisAsyncContext *c, int status)
{
    LOG4_TRACE("%s()", __FUNCTION__);
...
bool OssWorker::OnRedisDisconnect(const redisAsyncContext *c, int status)
{
...
bool OssWorker::OnRedisCmdResult(redisAsyncContext *c, void *reply, void *privdata)
{
    LOG4_DEBUG("%s()", __FUNCTION__);
    std::map<redisAsyncContext*, tagRedisAttr*>::iterator attr_iter = m_mapRedisAttr.find((redisAsyncContext*)c);
    if (attr_iter != m_mapRedisAttr.end())
...
bool OssWorker::OnRedisClusterCmdResult(redisClusterAsyncContext *acc, void *reply, void *privdata)
{
    LOG4_DEBUG("%s()", __FUNCTION__);
...
const std::string& OssWorker::GetWorkerIdentify()
{
...
bool OssWorker::RegisterCallback(MysqlStep* pMysqlStep)
{
#define iMysqlContextMapMaxSize (10)
	...
	if (ctx_iter != m_mapMysqlContext.end())//每个连接符对应最多10连接
	{
		...
		loss::MysqlAsyncConn* pMysqlConn(NULL);
...
bool OssWorker::RegisterCallback(loss::MysqlAsyncConn* pMysqlContext, MysqlStep* pMysqlStep)
{
    LOG4_TRACE("%s()", __FUNCTION__);
    if (pMysqlStep == NULL)
    {
    ...
    {
    	pMysqlStep->SetLabor(this);
		pMysqlStep->SetLogger(&m_oLogger);
		pMysqlStep->SetRegistered();
		LOG4_TRACE("register pMysqlStep ok");
    }
    if (pMysqlStep->m_strCmd.size() > 0)
    {
    	LOG4_TRACE("succeed in add mysql cmd: %s:%u uiAddTaskCounter:%llu",
    			pMysqlStep->m_strCmd.c_str(),pMysqlStep->m_uiCmdType,uiAddTaskCounter);
    	loss::SqlTask *task = new loss::SqlTask(pMysqlStep->m_strCmd,(loss::eSqlTaskOper)pMysqlStep->m_uiCmdType,
    			new CustomMysqlHandler(pMysqlStep,this));
...
bool OssWorker::AddMysqlContextAddr(MysqlStep* pMysqlStep, loss::MysqlAsyncConn* ctx)
{
    ...
    {
    	std::set<loss::MysqlAsyncConn*> set;
    	set.insert(ctx);
    	m_mapMysqlContext.insert(std::pair<std::string, std::pair<std::set<loss::MysqlAsyncConn*>::iterator,std::set<loss::MysqlAsyncConn*> > >
    	(szIdentify,std::make_pair(set.begin(),set)));
        std::map<loss::MysqlAsyncConn*, std::string>::iterator identify_iter = m_mapMysqlContextIdentify.find(ctx);
        if (identify_iter == m_mapMysqlContextIdentify.end())
        {
        	m_mapMysqlContextIdentify.insert(std::pair<loss::MysqlAsyncConn*, std::string>(ctx, szIdentify));
        }
        else
        {
            identify_iter->second = szIdentify;
...
void OssWorker::DelMysqlContextAddr(loss::MysqlAsyncConn* ctx)
{
    std::map<loss::MysqlAsyncConn*, std::string>::iterator identify_iter = m_mapMysqlContextIdentify.find(ctx);
    if (identify_iter != m_mapMysqlContextIdentify.end())
    {
...
//接入服务器使用的对外接口(只为了支持websocket接口，未使用的则不需要使用本接口)
...
bool OssWorker::SendToProxyCallBack(oss::Session* pSession,const DataMem::MemOperate* pMemOper,
		StorageCallbackSession callback,bool boPermanentSession,const std::string &nodeType,uint32 uiCmd,int uiModFactor)
{
	if (!pSession)
	{
		LOG4CPLUS_ERROR_FMT(GetLogger(),"pSession empty!");
		return false;
	}
	StepNodeAccess* pStep = new StepNodeAccess(pMemOper);
    if (pStep == NULL)
    {
    ...
    pStep->SetProxyCallBack(callback,pSession,boPermanentSession,nodeType,uiCmd,uiModFactor);
...
bool OssWorker::SendToProxyCallBack(oss::Step* pUpperStep,const DataMem::MemOperate* pMemOper,
		StorageCallbackStep callback,const std::string &nodeType,uint32 uiCmd,int uiModFactor)
{
	if (!pUpperStep)
	{
		LOG4CPLUS_ERROR_FMT(GetLogger(),"pUpperStep null!");
		return false;
	}
	if (!pUpperStep->IsRegistered())
	{
		...
		{
			LOG4CPLUS_ERROR_FMT(GetLogger(),"RegisterCallback(pUpperStep) error!");
			delete pUpperStep;
			pUpperStep = NULL;
			return(false);
		}
		LOG4CPLUS_TRACE_FMT(GetLogger(),"RegisterCallback(pUpperStep)");
	}
	StepNodeAccess* pStep = new StepNodeAccess(pMemOper);
    if (pStep == NULL)
    {
    ...
    pStep->SetProxyCallBack(callback,pUpperStep,nodeType,uiCmd,uiModFactor);
...
bool OssWorker::SendToCallBack(oss::Session* pSession,uint32 uiCmd,const std::string &strBody,
		StandardCallbackSession callback,bool boPermanentSession,const std::string &nodeType,int uiModFactor)
{
	StepNodeAccess* pStep = new StepNodeAccess(strBody);
    if (pStep == NULL)
    {
        LOG4CPLUS_ERROR_FMT(GetLogger(),"new StepNodeAccess() error!");
        delete pStep;
    ...
    pStep->SetCallBack(callback,pSession,boPermanentSession,nodeType,uiCmd,uiModFactor);
    if (oss::STATUS_CMD_RUNNING != pStep->Emit(ERR_OK))
    {
        DeleteCallback(pStep);
        return(false);
...
bool OssWorker::SendToCallBack(oss::Step* pUpperStep,uint32 uiCmd,const std::string &strBody,
		StandardCallbackStep callback,const std::string &nodeType,int uiModFactor)
{
	if (!pUpperStep)
	{
		LOG4CPLUS_ERROR_FMT(GetLogger(),"pUpperStep null!");
...
bool OssWorker::SendTo(const tagMsgShell& stMsgShell, const MsgHead& oMsgHead, const MsgBody& oMsgBody)
{
    ...
    else
    {
        ...
        if (ptagConnectionAttr->ulSeq == stMsgShell.ulSeq && ptagConnectionAttr->iFd == stMsgShell.iFd)
        {
            ...
            if (loss::CODEC_PROTOBUF == ptagConnectionAttr->eCodecType)//内部协议需要检查连接过程
            {
...
bool OssWorker::AutoRedisCluster(const std::string& sAddrList, RedisStep* pRedisStep)
{
...
bool OssWorker::AutoMysqlCmd(MysqlStep* pMysqlStep)
{
	LOG4_TRACE("%s() AutoMysqlCmd(%s, %d)", __FUNCTION__, pMysqlStep->m_strHost.c_str(), pMysqlStep->m_iPort);
	struct ev_loop *loop = m_loop;
	loss::MysqlAsyncConn * pConn = loss::mysqlAsyncConnect(pMysqlStep->m_strHost.c_str(),
...
bool OssWorker::Host2Addr(const std::string & strHost,int iPort,struct sockaddr_in &stAddr,bool boRefresh)
{
    stAddr.sin_family = AF_INET;
    stAddr.sin_port = htons(iPort);
    stAddr.sin_addr.s_addr = inet_addr(strHost.c_str());
...
bool OssWorker::HttpsGet(const std::string & strUrl, std::string & strResponse,
        const std::string& strUserpwd,loss::CurlClient::eContentType eType, const std::string& strCaPath,int iPort)
{
    loss::CurlClient curlClient;
...
bool OssWorker::HttpsPost(const std::string & strUrl, const std::string & strFields,
        std::string & strResponse,const std::string& strUserpwd,loss::CurlClient::eContentType eType,const std::string& strCaPath,int iPort)
{
	loss::CurlClient curlClient;
...
void OssWorker::LoadSo(loss::CJsonObject& oSoConf,bool boForce)
{
...
tagSo* OssWorker::LoadSoAndGetCmd(int iCmd, const std::string& strSoPath, const std::string& strSymbol, int iVersion)
{
    ...
    Cmd* pCmd = pCreateCmd();
    LOG4_TRACE("%s() strSoPath:%s pHandle:%p pCreateCmd:%p pCmd:%p",
                __FUNCTION__,strSoPath.c_str(),pHandle,pCreateCmd,pCmd);
    if (pCmd != NULL)
    {
        pSo = new tagSo();
        if (pSo != NULL)
...
void OssWorker::UnloadSoAndDeleteCmd(int iCmd)
{
    ...
    if (mapSoIt != m_mapSo.end())
    {
        LOG4_INFO("succeed in unloading(%s) strLoadTime(%s),strNowTime(%s)",
                                mapSoIt->second->strSoPath.c_str(),mapSoIt->second->strLoadTime.c_str(),loss::GetCurrentTime(20).c_str());
...
void OssWorker::LoadModule(loss::CJsonObject& oModuleConf,bool boForce)
{
    LOG4_TRACE("%s()", __FUNCTION__);
...
void OssWorker::UnloadSoAndDeleteModule(const std::string& strModulePath)
{
    ...
    {
        LOG4_INFO("succeed in unloading(%s) strLoadTime(%s),strNowTime(%s)",
                mapMoIt->second->strSoPath.c_str(),mapMoIt->second->strLoadTime.c_str(),loss::GetCurrentTime(20).c_str());
...
bool OssWorker::AddIoReadEvent(tagConnectionAttr* ptagConnectionAttr)
{
    LOG4_TRACE("%s()", __FUNCTION__);
...
bool OssWorker::AddIoWriteEvent(tagConnectionAttr* ptagConnectionAttr)
{
    LOG4_TRACE("%s()", __FUNCTION__);
    ev_io* io_watcher = NULL;
	if (NULL == ptagConnectionAttr->pIoWatcher)
...
bool OssWorker::RemoveIoWriteEvent(tagConnectionAttr* ptagConnectionAttr)
{
    LOG4_TRACE("%s()", __FUNCTION__);
	if (NULL != ptagConnectionAttr->pIoWatcher)
	{
...
bool OssWorker::Dispose(loss::MysqlAsyncConn *c, loss::SqlTask *task, MYSQL_RES *pResultSet)
{
	std::map<uint32, Step*>::iterator step_iter;
