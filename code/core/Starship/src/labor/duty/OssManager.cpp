...
bool OssManager::AddIoReadEvent(tagConnectionAttr* ptagConnectionAttr)
{
...
bool OssManager::AddIoWriteEvent(tagConnectionAttr* ptagConnectionAttr)
{
    LOG4_TRACE("%s(fd %d)", __FUNCTION__, ptagConnectionAttr->iFd);
    ev_io* io_watcher = NULL;
	if (NULL == ptagConnectionAttr->pIoWatcher)
...
bool OssManager::RemoveIoWriteEvent(tagConnectionAttr* ptagConnectionAttr)
{
    LOG4_TRACE("%s", __FUNCTION__);
    ev_io* io_watcher = NULL;
...
void OssManager::RefreshServer(bool boForce)
{
...
bool OssManager::DisposeDataAndTransferFd(const tagMsgShell& stMsgShell, const MsgHead& oInMsgHead, const MsgBody& oInMsgBody,
...
{
    LOG4_DEBUG("%s(cmd %u, seq %u)", __FUNCTION__, oInMsgHead.cmd(), oInMsgHead.seq());
    loss::CBuffer* pSendBuff = ptagConnectionAttr->pSendBuff;
	loss::CBuffer* pWaitForSendBuff = ptagConnectionAttr->pWaitForSendBuff;
    int iErrno = 0;
