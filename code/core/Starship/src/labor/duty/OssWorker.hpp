...
public:
    ...
    virtual const loss::CJsonObject& GetCustomConf() const {return(m_oCustomConf);}
...
public:
	...
	virtual bool RegisterCallback(loss::MysqlAsyncConn* pMysqlContext, MysqlStep* pMysqlStep);
	virtual bool AddMysqlContextAddr(MysqlStep* pMysqlStep, loss::MysqlAsyncConn* ctx);
	virtual void DelMysqlContextAddr(loss::MysqlAsyncConn* ctx);
public:
    ...
    virtual bool HttpsGet(const std::string & strUrl, std::string & strResponse,
            const std::string& strUserpwd = "",loss::CurlClient::eContentType eType = loss::CurlClient::eContentType_none,
	...
	bool Dispose(loss::MysqlAsyncConn *c, loss::SqlTask *task, MYSQL_RES *pResultSet);
protected:
	//初始化
    ...
    //动态库
    void LoadSo(loss::CJsonObject& oSoConf,bool boForce=false);
    ...
    void LoadModule(loss::CJsonObject& oModuleConf,bool boForce=false);
...
private:
    ...
    //mysql节点连接
    typedef std::map<std::string, std::pair<std::set<loss::MysqlAsyncConn*>::iterator,std::set<loss::MysqlAsyncConn*> > > MysqlContextMap;
    MysqlContextMap m_mapMysqlContext;       ///< mysql连接，key为identify(192.168.16.22:9988形式的IP+端口)
	std::map<loss::MysqlAsyncConn*, std::string> m_mapMysqlContextIdentify;    ///< mysql标识，与m_mapMysqlContext的key和value刚好对调
