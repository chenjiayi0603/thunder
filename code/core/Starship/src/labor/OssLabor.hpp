...
//访问存储回调
typedef void (*StorageCallbackSession)(const DataMem::MemRsp &oRsp,oss::Session*pSession);
typedef void (*StorageCallbackStep)(const DataMem::MemRsp &oRsp,oss::Step*pStep);
//访问一般节点回调
typedef void (*StandardCallbackSession)(const MsgHead& oInMsgHead,const MsgBody& oInMsgBody,void* data,oss::Session*pSession);
typedef void (*StandardCallbackStep)(const MsgHead& oInMsgHead,const MsgBody& oInMsgBody,void* data,oss::Step*pStep);
...
public:     // Worker相关设置（由Cmd类或Step类调用这些方法完成数据交互，Worker类必须重新实现这些方法，Manager类可不实现）
    ...
    int CoroutineNew(loss::coroutine_func func,void *ud);
