...
net::E_CMD_STATUS StepGetConfig::Emit(int iErrno, const std::string& strErrMsg, const std::string& strErrShow)
{
    ...
    else
    {
        ...
        GetLabor()->SendTo(m_tagMsgShell, oHttpMsg);
...
net::E_CMD_STATUS StepGetConfig::Callback(const net::tagMsgShell& stMsgShell, const MsgHead& oInMsgHead, const MsgBody& oInMsgBody, void* data)
{
	...
	if (oRes.ParseFromString(oInMsgBody.body()))
	{
		...
		oHttpMsg.set_http_minor(m_iHttpMinor);
		util::CJsonObject oResponseData;
		oResponseData.Add("code", oRes.err_no());
		oResponseData.Add("msg", oRes.err_msg());
		if (net::ERR_OK == oRes.err_no())
		{
