...
/**
 * @brief 告知对端己方Worker进程信息
 * @note 在作为客户端发起对一个服务端的连接，当对端返回连接成功信息时，启动 StepTellWorker向对端发送己方Worker信息，并等待对端回复对方Worker信息回调。
 */
class StepTellWorker : public Step
{
public:
	explicit StepTellWorker(const tagMsgShell& stMsgShell);
    virtual ~StepTellWorker() = default;
