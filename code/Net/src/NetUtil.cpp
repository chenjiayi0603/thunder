...
void RunClock::EndClock()
{
	...
	{
		gettimeofday(&m_tvRunEndClock,NULL);
		float useTime = LastUseTime();//ms
		if (m_uiErrorTime > 0 && m_uiErrorTime <= useTime)
		{
			LOG4_ERROR("%s() %s use time(%lf) ms",__FUNCTION__,m_desc,useTime);
		}
		else if (m_uiWarningTime > 0 && m_uiWarningTime <= useTime)
		{
			LOG4_WARN("%s() %s use time(%lf) ms",__FUNCTION__,m_desc,useTime);
		}
		else
		{
			LOG4_TRACE("%s() %s use time(%lf) ms",__FUNCTION__,m_desc,useTime);
		}
		boStart = false;
	}
}
