                ...
                const MsgBody& oInMsgBody)
{
    ...
    LOG4_TRACE("CmdUpdateConfig body(%s)",oInMsgBodyStr.c_str());
    //更新服务器配置文件
...
bool CmdUpdateConfig::ReadConfig()
{
    ...
    LOG4_TRACE("ReqConfigType(%d).CONF FILE = %s.ReqConfigContent(%s).",
    ...
    {//修改配置文件内容(这里是检查服务器配置)
        //需要更新的字段的内容
    	LOG4_TRACE("before update CONF FILE CONTENT= %s.",oLocalConfJson.ToFormattedString().c_str());
    	{//检查so
            ...
            {
                LOG4_TRACE("so don't exist.");
            }
            else
            {
                LOG4_TRACE("configUpdateContentSo:%s",configUpdateContentSo.ToString().c_str());
                ...
                {
                    LOG4_TRACE("don't need to update so(%s)",configUpdateContentSo.ToFormattedString().c_str());
                }
            }
    	}
    	{//检查log_level
            ...
            {
                LOG4_TRACE("log_level don't exist");
            }
            else
            {
            	int iLocalLogLevel(0);//本地日志等级
                LOG4_TRACE("log_level:%d",iLocalLogLevel);
                if(oLocalConfJson.Get("log_level",iLocalLogLevel) && (iLocalLogLevel != iUpdateLogLevel))
                {
                    LOG4_TRACE("update log_level(%d)",iUpdateLogLevel);
                    oLocalConfJson.Replace("log_level",iUpdateLogLevel);
                }
                else
                {
                    LOG4_TRACE("don't need to update log_level(%d)",iUpdateLogLevel);
                }
            }
    	}
    	{//检查module
            ...
            {
                LOG4_TRACE("module don't exist.");
            }
            else
            {
                LOG4_TRACE("module:%s",configUpdateContentModule.ToString().c_str());
                ...
                {
                    LOG4_TRACE("don't need to update module(%s)",configUpdateContentModule.ToFormattedString().c_str());
                }
            }
    	}
		oConfJsonStr = oLocalConfJson.ToFormattedString();
		LOG4_TRACE("after update CONF FILE CONTENT= %s.",oConfJsonStr.c_str());
    }
    else
    {
        LOG4_TRACE("before update CONF FILE CONTENT= %s.",
    ...
    {//写入配置文件内容
    	...
    	if (fin.good())
		{
			...
			fin.flush();
			fin.close();
			LOG4_TRACE("update strConfFile(%s) ok",strConfFile.c_str());
		}
		else
		{
			//配置信息流读取失败
