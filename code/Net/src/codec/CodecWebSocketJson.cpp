...
//    int iErrno = 0;
    LOG4_TRACE("cmd %u, seq %u, msgbody_len %u", oMsgHead.cmd(), oMsgHead.seq(),oMsgHead.msgbody_len());
    //在逻辑层处理心跳
	#ifdef HTTP_PING
        ...
        if (pConn->pRecvBuff->ReadableBytes() >= 5)//目前只支持Get post 的初始化握手协议
        {
                            ...
                            || memcmp(pReadAddr, "POST ", 5) == 0)
            {
                ...
                if (CODEC_STATUS_OK == eCodecStatus)
                {
                    ...
                    {
                        LOG4_TRACE("%s()　CodecWebSocketJson　need to init connect status with http head Upgrade for websocket",__FUNCTION__);
                        return CODEC_STATUS_ERR;
                    }
                }
                return eCodecStatus;
