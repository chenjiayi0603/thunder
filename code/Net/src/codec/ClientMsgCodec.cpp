...
//        pBuff->Compact(8192);
    ...
    LOG4_TRACE("oMsgBody.ByteSize() = %d, iHadWriteLen = %d(compress or encrypt maybe)", oMsgBody.ByteSize(), iHadWriteLen);
//    pBuff->Compact(8192);
    return(CODEC_STATUS_OK);
}
...
E_CODEC_STATUS ClientMsgCodec::Decode(tagConnectionAttr* pConn,MsgHead& oMsgHead, MsgBody& oMsgBody)
{
    ...
    if (eConnectStatus_init == pConn->ucConnectStatus)//连接状态处理为解码一个消息成功，则连接状态完成
    {
        ...
        {
            LOG4_TRACE("%s()　ClientMsgCodec　need to init connect status with private pb request",__FUNCTION__);
        }
    }
    return status;
}
