 ...
 * Project:  Net
 * @file     IoBackend.hpp
 * @brief    I/O backend abstraction (ev / io_uring) — 策略模式接口
 * @author   cjy
 * @date:    2026年5月11日
 * @note
 * Modify history:
 ...
 *                     支撑一套 Worker 代码适配 ev/io_uring 两种后端。
 ******************************************************************************/
 ...
 * @note
 *   一套 Worker 代码零分支适配 ev / io_uring / ev/io_uring 三种底层。
...
public:
     ...
     * @param ip           监听 IP
     * @param port         监听端口
     * @param boReusePort  是否设置 SO_REUSEPORT（不支持时静默忽略）
