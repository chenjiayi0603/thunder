...
public:
    ...
    net::E_CMD_STATUS Callback(const net::tagMsgShell& stMsgShell,const MsgHead& oInMsgHead,const MsgBody& oInMsgBody,void* data = nullptr);
    E_CMD_STATUS Callback(const tagMsgShell& stMsgShell,const HttpMsg& oHttpMsg,void* data = nullptr);
	...
	//错误信息
	int m_iErrno = 0;
	std::string m_strStepDesc = "StepState";
...
protected:
	...
	uint8 m_uiTimeOutRetry = 0;//是否超时重新尝试过程

	uint32 m_uiStateVecNum = 0;//状态过程计数
    uint32 m_uiState = 0;//状态机
    uint32 m_uiLastState = 0; //上一个状态机
    int m_uiNextState = -1;//指定下一个状态机
    bool m_boJumpState = false;//不等待本状态回调，直接运行下一个状态
private:
	StateFunc m_StateVec[StepStateVecSize] = {0};//状态过程函数
