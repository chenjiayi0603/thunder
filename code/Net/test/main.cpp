...
tagConnectionAttr* CreateFdAttr(int iFd, uint32 ulSeq, util::E_CODEC_TYPE eCodecType)
{
    LOG4_TRACE("%s(iFd %d, seq %lu, codec %d)", __FUNCTION__, iFd, ulSeq, eCodecType);
...
bool AddIoReadEvent(int iFd)
{
    ...
    auto iter =  g_mapFdAttr.find(iFd);
    if (iter != g_mapFdAttr.end())
    {
        if (nullptr == iter->second->pIoWatcher)
        {
...
bool DelEvents(ev_io** io_watcher_addr)
{
    ...
    delete pData;
    (*io_watcher_addr)->data = nullptr;
    delete (*io_watcher_addr);
    (*io_watcher_addr) = nullptr;
    io_watcher_addr = nullptr;
    return(true);
}
...
{
    LOG4_TRACE("%s()", __FUNCTION__);
    auto iter = g_mapFdAttr.find(stMsgShell.iFd);
    if (iter == g_mapFdAttr.end())
    {
...
void IoRead(struct ev_loop* loop, struct ev_io* watcher, int revents)
{
    ...
    else
    {
        ...
        {
            LOG4_TRACE("callback seq %lu not match the conn attr seq %lu",
...
//                                conn_iter->second->pRecvBuff->ReadableBytes(), conn_iter->second->pRecvBuff->GetReadIndex());
                if (CODEC_STATUS_OK == eCodecStatus)
                {
                    ...
                    if (oInMsgHead.cmd() > 0 && oInMsgHead.seq() > 0)  // 基于TCP的自定义协议请求或带cmd、seq自定义头域的http请求
                    {
                        ...
                        if (conn_iter->second->szRemoteAddr != nullptr) // 来自客户端的请求
                        {
                            strClientIp = conn_iter->second->szRemoteAddr;
                            if (conn_iter->second->pClientData != nullptr)
                            {
        ...
        {
            LOG4_TRACE("fd %d closed by peer, error %d!",
                            pData->iFd, iErrno);
            DestroyConnect(conn_iter);
        }
        else
        ...
        if (conn_iter->second->ulSeq == stMsgShell.ulSeq)
        {
            ...
            if (CODEC_STATUS_OK == eCodecStatus)
            {
                ...
                if (iWriteLen < 0)
                {
                    ...
                    {
                        LOG4_TRACE("write len %d, error %d", iWriteLen, iErrno);
                        conn_iter->second->dActiveTime = ev_now(m_loop);
                        AddIoWriteEvent(stMsgShell.iFd);
                    }
                    else
...
bool Dispose(const std::string& strFromIp, const tagMsgShell& stMsgShell,
                ...
                MsgHead& oOutMsgHead, MsgBody& oOutMsgBody)
{
    LOG4_TRACE("%s(cmd %u, seq %lu)", __FUNCTION__, oInMsgHead.cmd(), oInMsgHead.seq());
    ...
    if (gc_uiCmdReq & oInMsgHead.cmd())    // 新请求
    {
        if ((gc_uiCmdBit & oInMsgHead.cmd()) == 507)
        {
            ...
            LOG4_TRACE("send cmd %d", oOutMsgHead.cmd());
    ...
    {
        LOG4_TRACE("receive callback, cmd = %d", oInMsgHead.cmd());
    }
    return(true);
}
