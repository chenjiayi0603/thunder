...
{
    char m_szDbCharSet[16] = {0};                 	// DB字符集
    char m_szDbHost[32] = {0};                    	// DB所在服务器IP
    char m_szDbUser[32] = {0};                    	// DB用户名
    char m_szDbPwd[32] = {0};                     	// DB用户密码
    char m_szDbName[32] = {0};                    	// DB库名
    unsigned int m_uiDbPort = 0;                	// DB端口
    unsigned int m_uiIdcId = 0;                     // 虚拟IDC ID，用于访问数据库时选择数据代理，如果是直连访问，无需理会此配置
    unsigned int uiTimeOut = 3;

    tagDbConnInfo() = default;
...
{
    unsigned char m_ucAccess = 0;               	// 访问方式,见 E_DB_ACCESS
    unsigned char m_ucDbType = 0;               	// DB类型
    tagDbConnInfo m_stDbConnInfo;				// DB连接信息

    tagDbConfDetail() = default;
