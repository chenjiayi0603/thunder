...
- [x] 端到端：`libProto.so` + `ModuleHello.so` 编通；`deploy/test_helloserver_wrk.sh` 压测入口可用（见附录 E）
...
- **生成**：`code/Proto/CMakeLists.txt` 中 `add_custom_command`（`protoc` → `patch_common_pb_h.py` → `cmake/ProtoPatchCommonPbCc.cmake` 修补 `common.pb.cc`）。兼容：`code/Proto/regen_cpp.sh` 实为 `cmake --build … --target Proto`。
...
- **修改任意 `.proto` 后**：`cmake --build build --target Proto`（或全量 `cmake --build build`）。
...
| 文件 | 说明 |
|------|------|
| `test_helloserver_wrk.sh` | 封装 `wrk`，默认 `HELLO_HOST=127.0.0.1`、`HELLO_PORT=27006`、`HELLO_PATH=/hello/hello`（与 `deploy/Hello/conf/Hello.json` 中 `access_*` 与 `module.url_path` 一致） |
| `wrk_helloserver.lua` | `POST` + `Content-Type: application/json`，body `{"option":"Echo"}`（与 `ModuleHello::TestMsg` 中 Echo 分支一致） |

**用法**：先 `./start_helloserver.sh`，再 `./test_helloserver_wrk.sh`。可通过环境变量覆盖线程/连接数/时长（见脚本内注释）。

**结果**：样例输出可记录在 `deploy/wrk_test_result.md`；若 wrk 统计中出现大量 **Non-2xx**，请核对服务端实际 HTTP 状态行/响应格式是否与 wrk 预期一致。
