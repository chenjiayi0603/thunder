...
> 生成时间：2026-03-20（v3 — Task 化重构 + 代码简化分析 + 架构对比 + TODO 清单）
...
Week 1-2:  Phase 1 — BaseTask 过渡期
             P0: 基础设施 + BaseTask + HttpTask + CoroutineTask + HttpAwaiter
             P1: when_all + SleepFor + Redis/Mysql Awaiter
             P2: Hello 迁移 + 测试
...
| 优先级 | 文件路径 | 原因 |
|--------|----------|------|
| 高 | `code/Net/src/step/sys_step/StepIoTimeout.hpp`、`StepIoTimeout.cpp` | Worker.cpp:1006 唯一调用已被注释，头文件仍被 include 但无实际使用 |
...
| `code/Net/src/labor/Worker.cpp:34` | 移除 `#include "StepIoTimeout.hpp"`（对应死代码清理） |
...
**Step 1: 基础设施文件**
- [ ] 创建 `code/Net/include/task/` 目录
- [ ] 新建 `TaskParam.hpp`
- [ ] 新建 `CancellationToken.hpp`
- [ ] 新建 `CallbackAwaiter.hpp`（含 `<T>` 和 `<void>` 特化）
- [ ] 新建 `HttpResponse.hpp`
- [ ] 为 `Coroutine20.hpp` 的 `Task<T>` 添加 `using value_type = T;`
- [ ] 4 个基础文件编译通过
- [ ] CallbackAwaiter 单元测试
...
**Step 2: BaseTask（过渡期继承 Step）**
- [ ] 新建 `BaseTask.hpp` + `BaseTask.cpp`
- [ ] BaseTask 类定义（继承 Step）
- [ ] `Emit()` override → 启动 `Run()` 协程
- [ ] `Callback()` override → `ResumeWithResponse()` 桥接
- [ ] `Timeout()` override → `OnTimeout()` 桥接
- [ ] Worker 兼容性验证（通过现有 `m_mapCallbackStep` 调度）

**Step 3: HttpTask**
- [ ] 新建 `HttpTask.hpp` + `HttpTask.cpp`
- [ ] HttpTask 发送方法实现（`HttpGet`/`HttpPost`/`SendTo`）
- [ ] HTTP 回调走 `BaseTask::Callback()` → `ResumeWithHttpResponse()`

**Step 4: CoroutineTask**
- [ ] 新建 `CoroutineTask.hpp`
- [ ] CoroutineTask 编译通过
- [ ] 最简业务 Task 继承测试

**Step 5: HttpAwaiter**
- [ ] 新建 `HttpAwaiter.hpp` + `HttpAwaiter.cpp`
- [ ] `HttpGetAwaiter::await_suspend()` 实现
- [ ] `HttpPostAwaiter::await_suspend()` 实现
- [ ] `co_await HttpGetAsync(url)` 返回 `HttpResponse` 验证
- [ ] CancellationToken 取消检查

**Step 6-8: 工具层**
- [ ] `WhenAll.hpp` — `when_all` 并发原语实现
- [ ] `SleepFor.hpp` + `SleepFor.cpp` — libev ev_timer 协程化
- [ ] `RedisCoAwaiter.hpp` + `RedisCoAwaiter.cpp` — Redis 协程化
- [ ] MysqlCoAwaiter（如需要）

**Step 9: Hello 示例迁移**
- [ ] 编写 `HttpRequestTask`（继承 CoroutineTask）
- [ ] ModuleHello 路由更新为使用 `HttpRequestTask`
- [ ] 保留旧 StepState 示例作对照
- [ ] 端到端功能测试通过
...
- [ ] HTTP 响应分发 → `m_mapCallbackTask` 查找 → `ResumeWithHttpResponse()`
- [ ] MySQL 响应分发适配
- [ ] Redis 响应分发适配
- [ ] fallback 到旧 `m_mapCallbackStep` 路径（过渡期兼容）
...
**高优先级 — 死代码移除**
- [ ] 移除 `StepIoTimeout.hpp` / `StepIoTimeout.cpp`，清理 Worker.cpp 中的 `#include`
- [ ] 移除 `Hello/ModuleHello/StepHttpRequestCo.hpp` / `StepHttpRequestCo.cpp`
...
**低优先级 — 旧协程路径**
- [ ] 移除 `USE_COROUTINE` 相关代码：`StepCo.*`、`dispatcher/Coroutine.*`、`Util/src/coroutine/*`
- [ ] 将 `StepStateFunc`/`StepFinalFunc` 类型定义从 `StepCo.hpp` 迁移到 `StepState.hpp`（在 StepState 废弃前）或 `BaseTask.hpp`
