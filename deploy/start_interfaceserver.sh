...
#   [4/4] HTTP 冒烟：GenKey（生成 token/key）→ VerifyKey（校验），路径 Interface→Center→LOGIC
#
# 工作目录：deploy/Interface，配置默认 conf/Interface.json。
#
# 用法: ./start_interfaceserver.sh
#       CONF=conf/Interface.json ./start_interfaceserver.sh
#
# 可选环境变量：
...
if [[ "${SKIP_CENTER_LOGIC}" != "1" ]]; then
  ...
  if ! _tcp_listening "${CENTER_PORT}"; then
    ...
    echo "提示: 端口 ${CENTER_PORT} 已在监听，跳过启动 Center"
  fi
  ...
  if ! _tcp_listening "${LOGIC_PORT}"; then
    ...
    if ! _tcp_listening "${LOGIC_PORT}"; then
      echo "错误: Logic 未在 ${LOGIC_PORT} 监听，见 ${DEPLOY_ROOT}/Logic/log/start_interfaceserver.log" >&2
      echo "常见原因: LD_LIBRARY_PATH 缺 libmariadb（本脚本已设置 code/3party/lib/mariadb）；或 plugins/CmdGetToken.so 未就绪" >&2
      exit 1
    fi
  else
    echo "提示: 端口 ${LOGIC_PORT} 已在监听，跳过启动 Logic"
  fi
...
# GenKey：POST 到无 query 的 URL（body 为空，不走 option 演示分支）；VerifyKey：POST 到 ?token=&key=
