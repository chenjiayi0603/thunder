...
for _i in $(seq 1 30); do pgrep -f 'Hello_wss_robot_' >/dev/null 2>&1 && break; sleep 1; done
if ! pgrep -f 'Hello_wss_robot_' >/dev/null 2>&1; then
    echo "FATAL: Hello_wss_robot never started"
    exit 1
fi
while pgrep -f 'Hello_wss_robot_' >/dev/null 2>&1; do sleep 5; done
echo "FATAL: Hello_wss_robot exited, container will restart"
exit 1
