...
./bin/Hello ./conf/MqttBroker.json
EXIT=$?
if [ $EXIT -ne 0 ]; then
    echo "FATAL: MqttBroker exited with code $EXIT"
    exit $EXIT
fi
echo "MqttBroker daemonized; streaming logs + keep-alive..."
tail -f log/MqttBroker_robot.log 2>/dev/null &
