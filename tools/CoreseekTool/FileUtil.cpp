...
uint32 ExtractFileName(const char* sFilePath, char* sNameBuf, uint32 dwBufLen)
{
	...
	{
		uint32 dwNameLen = sNameEnd - sNameStart+1;
		if ( dwBufLen > 0 )
		{
...
uint32 ExtractFileNameOnly(const char* sFileName, char* sNameBuf, uint32 dwBufLen)
{
	//文件名为空则直接返回0
	if ( !*sFileName )
	{
...
uint32 ExtractFileDirectory(const char* sFilePath, char* sDirBuf, uint32 dwBufLen)
{
	const char* sDirEnd = sFilePath + strlen(sFilePath) - 1;
	while (sDirEnd >= sFilePath && *sDirEnd != '/')
	{
		sDirEnd--;
	}
	if ( sDirEnd > sFilePath )
	{
	    uint32 dwNameLen = sDirEnd - sFilePath;
		if ( dwBufLen > 0 )
		{
			if ( dwBufLen > dwNameLen )
				dwBufLen = dwNameLen;
...
uint32 ExtractTopDirectoryName(const char* sDirPath, const char* *ppChildDirPath,   char* sDirName, uint32 dwBufLen)
{
	const char* sNameEnd;
	//跳过目录名称前连续的'/'
	...
	{
		uint32 dwNameLen = sNameEnd - sDirPath;
		if ( dwBufLen > 0 )
		{
			if ( dwBufLen > dwNameLen )
				dwBufLen = dwNameLen;
...
bool DeepCreateDirectory(const char* sDirPath)
{
	char sPath[4096];
	char* sPathPtr = sPath;
	uint32 dwNameLen, dwBufLen = sizeof(sPath) - 1;
...
int GetDirCommonFilesByExt(const char* sDirname,const char* sFileExt,int nFileExtLen,std::vector<std::string> &filesVec)
{
    DIR* dp;
    struct dirent* dirp;
