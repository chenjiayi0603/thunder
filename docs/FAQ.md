...
> 从 [`docs/README.md`](README.md) 独立出来的 FAQ。适合已经跑通 Quick Start、想理解框架设计决策的读者。

---

## 性能概览

> 🔥 **Thunder 存在的理由：比 Nginx 更快、更灵活。**
>
> 完整基准测试报告 → [`performance/10-vs-nginx-benchmark-20260610.md`](performance/10-vs-nginx-benchmark-20260610.md)

| 场景 | Thunder | Nginx / 旧方案 |
|------|:----:|:----:|
| HTTP 1KB RPS | 232k | Nginx 191k (+21%) |
| HTTP 64B 延迟 | 220 μs | Nginx 466 μs |
| HTTPS 64B 延迟 | 402 μs | Nginx 752 μs |
| ThreadPool work-stealing | 543 ns/op | 旧 MPMC 队列 1374 ns/op |
...
net::AsyncTask AsyncBody(net::StepCo20& st) {
    co_await self.HttpGetAsync("http://baidu.com");
    co_await self.HttpGetAsync("http://sogou.com");
    co_await self.HttpGetAsync("http://qq.com");
    co_return;
}
```
...
ThreadPool offload (任务并行):
  请求 A → threadpool.commit(workA)
  请求 B → threadpool.commit(workB)
```
...
因为 AsyncTask 是**栈对象**，协程帧的释放由 AsyncTask 的析构函数控制：
...
// 内部：await_suspend → redisAsyncConnect → redisAsyncCommand
...
| 场景 | 数据 | 说明 |
|------|:----:|:----:|
| HTTP 空响应 (asio_uring) | 235k QPS | Nginx 214k |
| HTTPS 空响应 (asio_uring) | 133~141k QPS | wrk c100, 1 Worker 绑核 |
| Redis 协程 (单连接) | 12k QPS | 无连接池 |
| ThreadPool 队列 vs mutex | 2.5×~3.7× | ConcurrentQueue 比 mutex+queue 快 |
| ThreadPool vs TBB CPU密集 | 等同 | 两者均满 20 核 |
| 异步 Redis (hiredis async) | 29M QPS | 同步 103k |
| Work-stealing 加速比 | 543 ns/op | 旧 MPMC 队列 1374 ns/op |
...
2. **无锁线程池**：`moodycamel::ConcurrentQueue` 替代 `queue + mutex`，多生产者 push/pop 延迟从 ~3−4μs 降到 ~1μs
...
> 📖 更多设计细节: [`architecture/`](architecture/) · 性能数据: [`performance/`](performance/)
