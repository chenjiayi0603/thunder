...
**好处**: fork 后父子进程共享同一物理页——Manager 写入 slot[5],Worker 立即能看到,零拷贝。
...
每个 slot 固定大小(默认 4096 字节)。超过的消息被拒绝(返回 false)。这是设计取舍:**固定大小换零拷贝 + 无锁**。

如果需要大消息,上层负责分片(当前 Thunder 未实现分片)。
