...
## 13. 五种 I/O 后端性能排序 (实测数据, 2026-06-01)

### 性能层级

```
真 DPDK PMD  >  Ring PMD  >  AF_PACKET  >  原生 socket  >  TAP PMD
(ixgbe/mlx5)    (纯内存)     (旁路协议栈)    (epoll/ev)     (虚拟设备)

  10-20x           ∞            3-5x           1x (基准)      0.5x
需专用网卡     无网络意义      任何网卡        当前后端       调试用
```

### 各后端实测数据

| # | 后端 | 性能 (vs ev) | 网卡要求 | 真实网络I/O | 数据路径 |
|---|------|-------------|---------|-----------|---------|
| 1 | **真 DPDK PMD** | 10-20x | ixgbe/i40e/mlx5 | ✅ | DMA→大页→VFIO→网卡 |
| 2 | **Ring PMD** | ∞ (纯 memcpy) | 无 | ❌ | ring buffer (用户态内存) |
| 3 | **AF_PACKET** | 3-5x | **任何网卡** | ✅ | af_packet→内核驱动→网卡 |
| 4 | **原生 socket** | 1x (基准) | 任何 | ✅ | socket→内核TCP/IP→驱动→网卡 |
| 5 | **TAP PMD** | 0.5x (更慢) | 无 | ✅ | TAP→内核转发→网卡 |

### 实测数据 (本机 ThinkBook 14 G4+, I219-V, ev 后端)

```
wrk ev/epoll:           RPS=158,309  P50=630μs   (基准)
Ring PMD 虚拟端口:      双向收发 OK, API 验证通过
AF_PACKET L2 Echo:      666K PPS, 0 丢包
```

### 一个 Echo 请求的时间拆解 (ev 后端, 总计 ~65μs)

```
操作                      耗时     占比    可被 DPDK 优化?
──────────────────────────────────────────────────────────
epoll_wait + accept        5μs     8%     ✅
read/recv (syscall)        3μs     5%     ✅
HTTP FastPath 解析         2μs     3%     ❌ CPU
JSON 解析 (CJsonObject)    8μs    12%     ❌ CPU
ModuleHello 调度           3μs     5%     ❌ CPU
JSON 序列化                5μs     8%     ❌ CPU
write/send (syscall)       3μs     5%     ✅
TCP 状态机 (内核)          8μs    12%     ✅ 用户态替代
连接管理                   5μs     8%     ❌ 应用层
其他 (log/timer/...)      23μs    35%     ❌
──────────────────────────────────────────────────────────
DPDK 可优化部分           22μs    34%
```
...
**代码现状**: `DpdkIoBackend.{hpp,cpp}` 已写 (~700行), `#ifdef THUNDER_IO_DPDK` 双模式,
不影响当前编译。`tools/dpdk_afpacket_echo.c` 已验证 AF_PACKET PMD 在本机可用。

**缺的东西**: 用户态 TCP 栈。mTCP 只支持 DPDK 17.08, 跟本机 DPDK 25.11 不兼容。
替代方案: F-Stack (腾讯开源, FreeBSD TCP 栈, 支持 DPDK 20.11+)。

**集成工作量**: 装 F-Stack → 改 ~30 行 (`mtcp_*` → `ff_*`) → CMake 加
`-DTHUNDER_IO_DPDK=ON` → 配置 `"io_backend": "dpdk"` → 跑通。

**硬件路径**:
- 开发/测试: AF_PACKET (本机 I219-V 即可, 不需要买硬件)
- 性能压测: 外接网卡 (ADT-Link ¥150 + Intel I350 ¥100)
- 生产极速: 物理机 + Intel X520/X710

---

## 14. 为什么 F-Stack 非要 DPDK 网卡 — 腾讯云场景分析

### 14.1 F-Stack 的出身

F-Stack 是腾讯云 **DNSPod 团队**开发的。2012 年腾讯云 DNS 受到大规模 DDoS 攻击，从千兆升万兆网卡后
内核网络栈成为瓶颈。他们试了 DPDK 之后决定基于 DPDK 开发下一代 DNS 服务器（DKDNS），后来把
FreeBSD 11 的 TCP/IP 栈移植到 DPDK 上，就成了 F-Stack。

**F-Stack 从第一天就在 IDC 机房的物理服务器上跑，从来没考虑过"没有 DPDK 网卡"的场景。**
...
腾讯云机房（一台宿主机）:
┌──────────────────────────────────────────────────────────┐
│  物理服务器，100GbE Intel X710 网卡                        │
│                                                          │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐               │
│  │ 客户VM-1  │  │ 客户VM-2  │  │ 客户VM-N  │              │
│  │ Nginx    │  │ Redis    │  │ 你的服务  │  ← 用户买的VM  │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘               │
│       │virtio        │virtio        │virtio               │
│       ▼              ▼              ▼                     │
│  ┌──────────────────────────────────────────────────┐    │
│  │  F-Stack（宿主机用户态）                           │    │
│  │  · 替所有 VM 处理 TCP（三次握手/重传/拥塞控制）     │    │
│  │  · vhost-user 共享内存直通 VM                     │    │
│  │  · 零拷贝：网卡 DMA → 用户态大页 → VM 内存         │    │
│  └─────────────────────┬────────────────────────────┘    │
│                        ▼                                  │
│  ┌──────────────────────────────────────────────────┐    │
│  │  DPDK PMD（VFIO 独占网卡）                         │    │
...
│                        ▼                                  │
│              ┌──────────────────┐                        │
│              │  Intel X710 网卡  │  ← DPDK 原生 PMD      │
│              │  100GbE 光纤口    │                        │
│              └──────────────────┘                        │
└──────────────────────────────────────────────────────────┘
```

### 14.3 为什么腾讯云不需要 AF_PACKET

| 腾讯云的需求 | 解决方案 | 为什么不是 AF_PACKET |
|-------------|---------|---------------------|
| 买网卡 | 直接买 DPDK 兼容的 X710 | IDC 采购，不是个人淘宝 |
| 网卡独占 | VFIO 绑走，不做内核网卡 | 宿主机有多张网卡，管理口和业务口分开 |
| CPU 轮询 | 独占 CPU 核 | 物理机 64 核，分几个给 DPDK 无所谓 |
| 性能 | 10M PPS / 核 | AF_PACKET 只有 ~500K PPS，差 20 倍 |
| 运维 | 专门的基础架构团队 | 有内核/网络工程师，管 hugepages/DPDK 绑定 |
| 成本 | 网卡几千块 | 相比服务器和带宽成本忽略不计 |

### 14.4 腾讯云的场景 vs 你的场景

| | 腾讯云 | 你 |
|------|--------|-----|
| 机器 | 物理服务器，64核 | 笔记本，20核 |
| 网卡 | Intel X710 100GbE | Intel I219-V 1GbE |
| 网卡兼容 DPDK | ✅ 原生 PMD | ❌ 无 PMD |
| 独占网口 | ✅ 管理口+业务口分开 | ❌ 只有一个口 |
| CPU 轮询 | ✅ 64核，分8个给 DPDK | ❌ 20核还要跑别的 |
| AF_PACKET | ❌ 不需要（性能不够） | ✅ 唯一能用的 |
| 团队 | 基础架构团队维护 | 一个人 |

### 14.5 结论

**F-Stack 是为 IDC 机房设计的，不是为个人开发机设计的。**

它假设你:
- 网卡是 DPDK 兼容的（不需要 AF_PACKET）
- 有多个网口（可以独占一个给 DPDK）
- 有足够 CPU 核跑轮询（100% CPU 无所谓）
- 有运维团队管内核参数

三个假设你都不满足。所以 F-Stack 不适合你的场景。

**你的路径**: AF_PACKET 旁路内核协议栈，TCP 留在内核或用轻量替代。

---

## 15. CLB 的路由 vs Thunder 的路由 — 为什么都有逻辑，但瓶颈不同
...
CLB 在处理每个请求时要做的:
  ...
  路由层 (CLB 自己):
    ...
    · 权重 (round-robin 下标 +1)
    · 灰度 (URL 匹配 → 新版本实例池)
    · Host 路由 (api.xxx.com → 后端A)
    · SSL 卸载 (TLS 解密 → 明文 HTTP)
...
```
一个 HTTP 请求在 CLB 里的时间:

  TCP 收包 + 组装          30μs   ← 网络 I/O (70%)
  查后端 hash 表            1μs    ← 路由逻辑
  检查健康状态              1μs    ← 路由逻辑
  选后端 (round-robin)      0.5μs  ← 路由逻辑
  TCP 发到后端              15μs   ← 网络 I/O
                          ─────
  总计                     ~48μs
  其中路由逻辑              2.5μs (5%)
  其中网络 I/O             45μs  (95%)
...
```
一个 Echo 请求在 Thunder 里的时间:

  TCP 收包 (内核)          5μs    ← 网络 I/O (8%)
  HTTP 解析                2μs    ← CPU
  JSON 解析                8μs    ← CPU (重点!)
  业务逻辑 (ModuleHello)    3μs    ← CPU
  JSON 序列化              5μs    ← CPU
  TCP 发包 (内核)          3μs    ← 网络 I/O (5%)
  TCP 状态机 + 杂项        39μs   ← 杂项
                          ─────
  总计                     ~65μs
  其中业务逻辑             13μs  (20%)
...
| | CLB | Thunder |
|---|---|---|
| 请求做什么 | 查表 + 转发 | JSON + 业务 + DB |
| 瓶颈在哪 | 网络 I/O (95%) | 业务逻辑 (20%) |
| 优化网络 I/O | ✅ 巨大收益 | ❌ 收益小 |
| F-Stack 值得吗 | ✅ 值得 | ❌ 不值得 |
| 应该优化什么 | DPDK/F-Stack | JSON 解析/Codec |
