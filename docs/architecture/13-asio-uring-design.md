...
实测结论（完整数据见 `docs/performance/11-io-backend-comparison.md` 与 `13-asio-uring-design.md#附录性能基准数据`）：
...
> 原独立文档 `performance/13-asio-uring-design.md#附录性能基准数据`，合并于此方便查阅。
...
| **native_uring** | **203k** | **427μs** | **−13%，延迟持平** | **−14%，延迟高 207μs** |
| **asio_uring** | **235k** | **220μs** | **+1%，延迟低 204μs** | 基准 |
| Nginx 1w | 214k | 466μs | — | — |
...
- **asio_uring 的核心优势**：ev_prepare 批量提交将 N 次 syscall 压为 1 次，HTTP 延迟 220μs（ev 424μs，Nginx 466μs）。
