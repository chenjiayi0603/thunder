...
### 9.1 日志写本地吗?会回滚还是一直增长?

**写本地**: 会,且 **3 个 Center 节点各存各的一份**(不是只主存),每条**先 fsync 落盘才回 ack**(不丢的前提)。崩溃重启就靠读回本地日志 + 快照恢复。

```
C1 磁盘            C2 磁盘            C3 磁盘
data/raft/log/    data/raft/log/    data/raft/log/
 [#1..#150]        [#1..#150]        [#1..#150]   ← 各一份, 内容一致
```

**一条日志分三段,两头各有一种"缩短"机制——所以既不无限增长、也不乱回滚**:

```
  已快照并删除        已提交(不可变)        未提交尾部
 ┌─────────────┐   ┌──────────────────┐   ┌──────────┐
 │ #1 .. #100  │   │ #101 ... #150     │   │ #151 #152│
 │ (压缩删掉)   │   │ (永不回滚)         │   │ (可能被覆盖)│
 └─────────────┘   └──────────────────┘   └──────────┘
      ▲前端压缩                                ▲尾部可回滚
  快照到#100后删旧日志                    换主时与新Leader冲突→截断重写
```

| 机制 | 在哪段 | 行为 |
|---|---|---|
| **① 前端压缩** | 已快照部分 | 攒够 N 条/定时 → 出快照(存当前状态)→ 删快照前日志 ⟹ **长度有界,不无限增长** |
| **② 尾部回滚** | 未提交尾部 | Follower 未提交的尾部若与新 Leader 冲突 → **截断重写** |
| **不回滚** | 已提交部分 | 过了多数派的日志**永不回滚**(Raft 安全保证) |

> **对 node_id 的含义**: 一旦某 node_id 的分配日志**被提交**,就钉死了,不因换主而丢或变——正是要 NuRaft 而非现状游标合并的根本原因。
