...
int main(int argc, char* argv[])
{
	...
	//协商密码测试
	{
		...
		//生成shareKey
		//uint8_t* calculate_ecdh_share_key(char* alicePubKey, char* alicePriKey);
		shareKey = calculate_ecdh_share_key((char*)ecdhPubkeyA.c_str(), (char*)ecdhPrivateKeyB.c_str());
		memset(tmpPub, 0 , 67);
		printf("\n--------B:sharekey:%s\n", hex2Str((char*)shareKey, 32, tmpPub));
		if(shareKey){free(shareKey);}
	}
